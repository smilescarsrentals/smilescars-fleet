...
export async function getHistory() {
  const rows = await q(`SELECT * FROM history ORDER BY timestamp DESC, id DESC`);
...
export async function getCarHistory({ plate }) {
  ...
  const rows = await q(
    `SELECT * FROM history WHERE lower(btrim(plate)) = ANY($1) ORDER BY timestamp DESC, id DESC`,
...
export async function getHistoryByStaff({ staffName }) {
  ...
  const rows = await q(`SELECT * FROM history WHERE lower(btrim(staff_name))=lower(btrim($1)) ORDER BY timestamp DESC, id DESC`, [staffName]);
  return { success: true, data: rows.map(mapHistory) };
}
