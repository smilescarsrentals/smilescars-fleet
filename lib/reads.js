...
export async function getStaffList() {
  const rows = await q(`SELECT value, role, phone, COALESCE(active,'TRUE') AS active, COALESCE(receives_all_reservation_reminders,'FALSE') AS receives_all_reservation_reminders, COALESCE(receives_driver_document_reminders,'FALSE') AS receives_driver_document_reminders FROM config WHERE type='Staff' AND value<>''`);
  return {
    success: true,
    staff: rows.map((r) => ({
...
const mapDriver = (r) => ({
  ...
  nationalId: S(r.national_id), address: S(r.address), notes: S(r.notes),
  active: String(r.active).trim().toUpperCase() !== "FALSE",
  createdAt: T(r.created_at), updatedAt: T(r.updated_at),
});
...
const mapDriverDocument = (r) => ({
  ...
  fileId: S(r.file_id), expiryDate: S(r.expiry_date), notes: S(r.notes),
  createdAt: T(r.created_at), updatedAt: T(r.updated_at),
});

export async function getDriverDocuments({ driverId } = {}) {
  if (!driverId) return { success: true, data: [] };
  const rows = await q(`SELECT * FROM driver_documents WHERE driver_id=$1 ORDER BY expiry_date ASC NULLS LAST, created_at DESC`, [driverId]);
