...
// Admin-only list for the license-photo cleanup review tool — every
// existing Driving License document with a file, so Ramzanali can review
// a before/after for each before anything gets replaced.
export async function getDriverLicenseDocsForCleanup(query) {
  await requireAdmin(query.staffName);
  const rows = await q(
    `SELECT dd.id, dd.file_id, d.name AS driver_name
       FROM driver_documents dd
       JOIN drivers d ON d.id = dd.driver_id
      WHERE dd.doc_type = 'Driving License' AND dd.file_id IS NOT NULL AND dd.file_id <> ''
      ORDER BY d.name ASC`
  );
  return {
    success: true,
    data: rows.map((r) => ({ id: r.id, driverName: r.driver_name, fileId: r.file_id, fileUrl: `/api?action=file&id=${encodeURIComponent(r.file_id)}` })),
  };
}
