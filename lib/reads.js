...
// car, joining the plate<->tracker pairing with yesterday's mileage and
// the last-synced location. Deliberately only confirmed cars — unmatched
// trackers/cars have nothing to show here and stay in the review sections.
export async function getTrackerOverviewTable() {
  const latest = await getLatestMileageDay();
  const [confirmed, mileageRows, locationRows] = await Promise.all([
    ...
    latest.day
      ? q(`SELECT plate, distance_m, over_limit FROM vehicle_mileage_daily WHERE day=$1`, [latest.day])
      : Promise.resolve([]),
  ...
  return { success: true, mileageDay: latest.day, data };
}
