...
import { q, q1, S, D, T, todayTZ, requireHRViewAccess, requireHRViewOrCOOAccess, HR_ENABLED_LOCATIONS, requireWorkflowViewAccess, requireAdmin } from "./core.js";
import { getDeviceList, getDeviceListCacheInfo } from "./tracksolid.js";
...
const mapHistory = (r) => ({
  id: r.id, timestamp: T(r.timestamp), plate: S(r.plate), type: S(r.type), action: S(r.action),
  ...
  isReplacement: String(r.is_replacement).trim().toUpperCase() === "TRUE",
  workOrderId: S(r.work_order_id),
  // Only present on a Returned/Transfer Completed row that had a fuel-gauge
  // photo attached (see markReturned + history_fuel_photos).
  fuelPhotoUrl: r.fuel_photo_file_id ? `/api?action=file&id=${encodeURIComponent(r.fuel_photo_file_id)}` : "",
});
...
const mapFuel = (r) => ({
  ...
  currentKm: r.current_km === null || r.current_km === undefined ? null : Number(r.current_km),
});
...
function mapAccident(r) {
  return {
    id: r.id, plate: r.plate, driverName: r.driver_name || "", accidentDate: D(r.accident_date),
    stationReport: r.station_report || "", injuriesOccurred: String(r.injuries_occurred).trim().toUpperCase() === "TRUE",
    details: r.details || "", reportedBy: r.reported_by, createdAt: T(r.created_at),
  };
}
export async function getAccidents() {
  const rows = await q(
    `SELECT a.*, (SELECT count(*) FROM accident_files f WHERE f.accident_id = a.id) AS file_count
       FROM accidents a ORDER BY a.accident_date DESC, a.created_at DESC`
  );
  return { success: true, data: rows.map((r) => ({ ...mapAccident(r), fileCount: Number(r.file_count) || 0 })) };
}
export async function getAccidentById({ id } = {}) {
  if (!id) return { success: false, error: "Accident ID required" };
  const row = await q1(`SELECT * FROM accidents WHERE id=$1`, [id]);
  if (!row) return { success: false, error: "Accident not found" };
  const files = await q(`SELECT * FROM accident_files WHERE accident_id=$1 ORDER BY uploaded_at ASC`, [id]);
  return {
    success: true,
    data: {
      ...mapAccident(row),
      files: files.map((f) => ({
        id: f.id, fileId: f.file_id, fileType: f.file_type, fileMimeType: f.file_mime_type || "",
        filename: f.filename || "", fileUrl: `/api?action=file&id=${encodeURIComponent(f.file_id)}`,
        uploadedBy: f.uploaded_by, uploadedAt: T(f.uploaded_at),
      })),
    },
  };
}
...
// fleet.garage is only ever a flat display label — the structured fields
// (service_location_type/external_vendor_id/external_vendor_location) live
// on the linked maintenance_log work order instead, not on fleet itself.
// Used to pre-fill the Fleet page's "Edit Garage" picker with the car's
// actual current selection, fetched on demand rather than joined into
// every getFleet() call (most cars aren't at Maintenance).
export async function getCurrentGarageAssignment({ plate } = {}) {
  if (!plate) return { success: false, error: "Plate is required" };
  const row = await q1(
    `SELECT service_location_type, internal_location, external_vendor_id, external_vendor_location
       FROM maintenance_log WHERE plate=$1 AND status <> 'Completed' ORDER BY date_opened DESC LIMIT 1`,
    [plate]
  );
  return {
    success: true,
    data: {
      serviceLocationType: row?.service_location_type || "Internal",
      internalLocation: row?.internal_location || "",
      externalVendorId: row?.external_vendor_id || "",
      externalVendorLocation: row?.external_vendor_location || "",
    },
  };
}
...
// ── History ─────────────────────────────────────────────────────────────────
// Left-joined so a return's fuel-gauge photo (if any) rides along with its
// history row instead of needing a second round-trip per row.
const HISTORY_SELECT = `SELECT h.*, hfp.file_id AS fuel_photo_file_id FROM history h
  LEFT JOIN history_fuel_photos hfp ON hfp.history_id = h.id`;
export async function getHistory() {
  const rows = await q(`${HISTORY_SELECT} ORDER BY h.timestamp DESC, h.id DESC`);
...
export async function getCarHistory({ plate }) {
  ...
  const rows = await q(
    `${HISTORY_SELECT} WHERE lower(btrim(h.plate)) = ANY($1) ORDER BY h.timestamp DESC, h.id DESC`,
...
export async function getHistoryByStaff({ staffName }) {
  ...
  const rows = await q(`${HISTORY_SELECT} WHERE lower(btrim(h.staff_name))=lower(btrim($1)) ORDER BY h.timestamp DESC, h.id DESC`, [staffName]);
  return { success: true, data: rows.map(mapHistory) };
}
...
export async function getStaffList() {
  const rows = await q(`SELECT value, role, phone, COALESCE(active,'TRUE') AS active, COALESCE(receives_all_reservation_reminders,'FALSE') AS receives_all_reservation_reminders, COALESCE(receives_driver_document_reminders,'FALSE') AS receives_driver_document_reminders, COALESCE(receives_insurance_reminders,'FALSE') AS receives_insurance_reminders, COALESCE(can_manage_drivers,'FALSE') AS can_manage_drivers, COALESCE(can_issue_fuel_voucher,'FALSE') AS can_issue_fuel_voucher, COALESCE(location,'') AS location, COALESCE(hr_access,'None') AS hr_access, COALESCE(is_coo,'FALSE') AS is_coo, COALESCE(workflow_access,'None') AS workflow_access FROM config WHERE type='Staff' AND value<>''`);
  return {
    success: true,
    staff: rows.map((r) => ({
      ...
      receivesInsuranceReminders: String(r.receives_insurance_reminders).trim().toUpperCase() === "TRUE",
      canManageDrivers: String(r.can_manage_drivers).trim().toUpperCase() === "TRUE",
      canIssueFuelVoucher: String(r.can_issue_fuel_voucher).trim().toUpperCase() === "TRUE",
      location: r.location || "",
      hrAccess: r.hr_access || "None",
      isCoo: String(r.is_coo).trim().toUpperCase() === "TRUE",
      workflowAccess: r.workflow_access || "None",
    })),
  };
}

// ── HR module ────────────────────────────────────────────────────────────
// Staff directory for the HR page — only Dar-tagged staff (the pilot's
// scope), regardless of active/inactive so past employees' records stay
// reachable. hasProfile lets the frontend show "not started" vs a filled
// profile without a second round-trip per row.
export async function getHRStaffList(query) {
  await requireHRViewAccess(query.staffName);
  const rows = await q(
    `SELECT c.value, c.role, c.phone, COALESCE(c.active,'TRUE') AS active,
            (p.staff_name IS NOT NULL) AS has_profile
       FROM config c
       LEFT JOIN staff_hr_profile p ON btrim(p.staff_name) = btrim(c.value)
      WHERE c.type='Staff' AND c.location = ANY($1)
      ORDER BY c.value`,
    [HR_ENABLED_LOCATIONS]
  );
  return {
    success: true,
    staff: rows.map((r) => ({
      name: r.value, role: r.role || "Staff", phone: r.phone || "",
...
export async function getHRStaffProfile(query) {
  await requireHRViewAccess(query.staffName);
  if (!query.targetStaffName) throw new Error("Target staff name is required");
  const basic = await q1(`SELECT value, role, phone, location FROM config WHERE type='Staff' AND btrim(value)=btrim($1)`, [query.targetStaffName]);
  if (!basic) throw new Error("Staff not found: " + query.targetStaffName);
  const profile = await q1(`SELECT * FROM staff_hr_profile WHERE btrim(staff_name)=btrim($1)`, [query.targetStaffName]);
  return {
    success: true,
    data: {
      name: basic.value, role: basic.role || "Staff", phone: basic.phone || "", location: basic.location || "",
      nationalId: profile?.national_id || "", contractType: profile?.contract_type || "",
      startDate: D(profile?.start_date), nextOfKinName: profile?.next_of_kin_name || "",
      nextOfKinRelationship: profile?.next_of_kin_relationship || "", nextOfKinPhone: profile?.next_of_kin_phone || "",
      notes: profile?.notes || "", updatedBy: profile?.updated_by || "", updatedAt: T(profile?.updated_at),
    },
  };
}

// Self-view — a plain staff member looking at their OWN profile, no
// hr_access needed (personal, read-only, mirrors the disciplinary/leave
// self-view pattern). Same shape as getHRStaffProfile so the frontend can
// reuse one display component for both.
export async function getMyProfile(query) {
  if (!query.staffName) throw new Error("Staff name is required");
  const basic = await q1(`SELECT value, role, phone, location FROM config WHERE type='Staff' AND btrim(value)=btrim($1)`, [query.staffName]);
  if (!basic) throw new Error("Staff not found: " + query.staffName);
  const profile = await q1(`SELECT * FROM staff_hr_profile WHERE btrim(staff_name)=btrim($1)`, [query.staffName]);
  return {
    success: true,
    data: {
      name: basic.value, role: basic.role || "Staff", phone: basic.phone || "", location: basic.location || "",
...
export async function getMyLeaveRequests(query) {
  if (!query.staffName) throw new Error("Staff name is required");
  const rows = await q(`SELECT * FROM staff_leave_requests WHERE btrim(staff_name)=btrim($1) ORDER BY submitted_at DESC`, [query.staffName]);
  return { success: true, data: rows.map(mapLeaveRequest) };
}

export async function getHRLeaveRequests(query) {
  await requireHRViewOrCOOAccess(query.staffName);
  const rows = await q(`SELECT * FROM staff_leave_requests ORDER BY submitted_at DESC`);
  return { success: true, data: rows.map(mapLeaveRequest) };
}

function mapLeaveRequest(r) {
  return {
    id: r.id, staffName: r.staff_name, leaveType: r.leave_type,
    startDate: D(r.start_date), endDate: D(r.end_date), daysRequested: r.days_requested != null ? Number(r.days_requested) : null,
    reason: r.reason || "", status: r.status,
    hrReviewedBy: r.hr_reviewed_by || "", hrReviewedAt: T(r.hr_reviewed_at), hrNotes: r.hr_notes || "",
    cooReviewedBy: r.coo_reviewed_by || "", cooReviewedAt: T(r.coo_reviewed_at), cooNotes: r.coo_notes || "",
    cancelledBy: r.cancelled_by || "", cancelledAt: T(r.cancelled_at),
    addedBy: r.added_by || "",
    submittedAt: T(r.submitted_at),
  };
}

function mapDisciplinary(r) {
  return { id: r.id, staffName: r.staff_name, date: D(r.date), type: r.type, description: r.description || "", issuedBy: r.issued_by || "", createdAt: T(r.created_at) };
}
export async function getMyDisciplinaryRecords(query) {
  if (!query.staffName) throw new Error("Staff name is required");
  const rows = await q(`SELECT * FROM staff_disciplinary WHERE btrim(staff_name)=btrim($1) ORDER BY date DESC`, [query.staffName]);
  return { success: true, data: rows.map(mapDisciplinary) };
}
export async function getHRDisciplinaryRecords(query) {
  await requireHRViewAccess(query.staffName);
  const rows = query.targetStaffName
    ? await q(`SELECT * FROM staff_disciplinary WHERE btrim(staff_name)=btrim($1) ORDER BY date DESC`, [query.targetStaffName])
    : await q(`SELECT * FROM staff_disciplinary ORDER BY date DESC`);
  return { success: true, data: rows.map(mapDisciplinary) };
}

// Default new-hire checklist, seeded once (on first view) per staff member
// rather than at add-staff time, so it also works retroactively for staff
// already in the system when onboarding tracking was introduced.
export const ONBOARDING_DEFAULT_ITEMS = [
  "Collect national ID / passport copy",
  "Signed employment contract on file",
  "Next of kin details collected",
  "Bank details collected for payroll",
  "System login / access set up",
  "Uniform / ID badge issued",
  "Orientation / induction completed",
];
function mapOnboardingItem(r) {
  return { id: r.id, staffName: r.staff_name, item: r.item, completed: String(r.completed).trim().toUpperCase() === "TRUE",
    completedBy: r.completed_by || "", completedAt: T(r.completed_at), sortOrder: r.sort_order };
}
export async function getOnboardingChecklist(query) {
  await requireHRViewAccess(query.staffName);
  if (!query.targetStaffName) throw new Error("Target staff name is required");
  const rows = await q(`SELECT * FROM staff_onboarding WHERE btrim(staff_name)=btrim($1) ORDER BY sort_order ASC, created_at ASC`, [query.targetStaffName]);
  return { success: true, data: rows.map(mapOnboardingItem) };
}

// ── Workflows module (invoice approvals) ────────────────────────────────
function mapWorkflowInvoice(r) {
  return {
    id: r.id, fileId: r.file_id, fileUrl: r.file_id ? `/api?action=file&id=${encodeURIComponent(r.file_id)}` : "",
    note: r.note || "", status: r.status, uploadedBy: r.uploaded_by, uploadedAt: T(r.uploaded_at),
    reviewedBy: r.reviewed_by || "", reviewedAt: T(r.reviewed_at), remarks: r.remarks || "", revision: r.revision,
    invoiceNumber: r.invoice_number || "", clientName: r.client_name || "",
    fileDeletedAt: T(r.file_deleted_at),
  };
}
export async function getWorkflowInvoices(query) {
  await requireWorkflowViewAccess(query.staffName);
  const rows = await q(`SELECT * FROM workflow_invoices ORDER BY uploaded_at DESC`);
  return { success: true, data: rows.map(mapWorkflowInvoice) };
}
export async function getWorkflowInvoiceEvents(query) {
  await requireWorkflowViewAccess(query.staffName);
  if (!query.invoiceId) throw new Error("Invoice ID is required");
  const rows = await q(`SELECT * FROM workflow_invoice_events WHERE invoice_id=$1 ORDER BY at ASC`, [query.invoiceId]);
  return { success: true, data: rows.map((r) => ({ id: r.id, action: r.action, by: r.by_staff, at: T(r.at), remarks: r.remarks || "" })) };
}

// ── Car insurance cover notes ───────────────────────────────────────────
function mapCoverNote(r) {
  return {
    id: r.id, plate: r.plate, fileId: r.file_id, fileUrl: `/api?action=file&id=${encodeURIComponent(r.file_id)}`,
    coverNoteNumber: r.cover_note_number || "", insurerName: r.insurer_name || "",
    startDate: D(r.start_date), expiryDate: D(r.expiry_date),
    uploadedBy: r.uploaded_by, uploadedAt: T(r.uploaded_at),
  };
}

// Open to anyone who can already view the car (matches getAllDriverDocuments'
// no-extra-gate convention) — only uploading/managing is Admin-only.
export async function getCoverNotesForCar(query) {
  if (!query.plate) return { success: false, error: "Plate is required" };
  const rows = await q(`SELECT * FROM car_insurance_cover_notes WHERE plate=$1 ORDER BY expiry_date DESC NULLS LAST, uploaded_at DESC`, [query.plate]);
  return { success: true, data: rows.map(mapCoverNote) };
}

// Admin-only overview for the Cover Notes tab: every plate in the fleet,
// whether it has a cover note on file, and what its current (furthest-
// expiry) one's status is — so after a bulk upload it's easy to see which
// cars still have nothing filed.
export async function getFleetCoverNoteStatus(query) {
  await requireAdmin(query.staffName);
  const rows = await q(`
    SELECT f.plate, f.type,
           cn.expiry_date AS current_expiry, cn.insurer_name AS current_insurer, cn.id AS current_cover_note_id
      FROM fleet f
      LEFT JOIN LATERAL (
        SELECT * FROM car_insurance_cover_notes c WHERE c.plate = f.plate ORDER BY c.expiry_date DESC NULLS LAST LIMIT 1
      ) cn ON true
     ORDER BY f.plate
  `);
  return {
    ...
    data: rows.map((r) => ({
      ...
      hasCoverNote: !!r.current_cover_note_id,
      currentExpiry: D(r.current_expiry), currentInsurer: r.current_insurer || "",
    })),
  };
}
...
// Auto-fill helper for Job Card Items added "from Supplier" (not stock) —
// looks at what we last actually paid this specific supplier for this
// exact item name, across any past work order. Zero-cost rows are
// excluded (several existing rows are incomplete placeholders, not real
// prices) so a bad old entry can't suggest "free" for something real.
export async function getPreviousSupplierPrice({ supplierVendorId, itemName } = {}) {
  if (!supplierVendorId || !itemName || !itemName.trim()) return { success: true, unitCost: null };
  const row = await q1(
    `SELECT unit_cost FROM maintenance_items
      WHERE supplier_vendor_id=$1 AND btrim(lower(item_name))=btrim(lower($2)) AND unit_cost > 0
      ORDER BY created_at DESC LIMIT 1`,
    [supplierVendorId, itemName]
  );
  return { success: true, unitCost: row ? Number(row.unit_cost) : null };
}
...
const mapDriver = (r) => ({
  ...
  active: String(r.active).trim().toUpperCase() !== "FALSE",
  createdAt: T(r.created_at), updatedAt: T(r.updated_at),
  leaveDaysAvailable: r.leave_days_available != null ? Number(r.leave_days_available) : null,
});
...
// Open to anyone who can see the Drivers page at all (same as
// getAllDriverDocuments/getDriversV2, no extra gate) — only adding a
// record or changing the leave-days number is restricted to
// requireDriverManageAccess, in lib/writes.js.
export async function getDriverLeaveRecords({ driverId } = {}) {
  if (!driverId) return { success: false, error: "Driver ID required" };
  const rows = await q(`SELECT * FROM driver_leave_records WHERE driver_id=$1 ORDER BY date DESC, logged_at DESC`, [driverId]);
  return {
    success: true,
    data: rows.map((r) => ({
      id: r.id, driverId: r.driver_id, date: D(r.date), endDate: D(r.end_date), leaveType: r.leave_type || "", daysUsed: Number(r.days_used),
      fileId: r.file_id || "", fileMimeType: r.file_mime_type || "", notes: r.notes || "",
      loggedBy: r.logged_by, loggedAt: T(r.logged_at),
    })),
  };
}
...
export async function getTrackerMatchSuggestions() {
  ...
  const cacheInfo = await getDeviceListCacheInfo();
  ...
  return {
    ...
    dataAsOf: cacheInfo.fetchedAt ? T(cacheInfo.fetchedAt) : null,
  };
}
...
// Most recent day the nightly sync actually saved data for — the
// Tracking page's "yesterday's mileage" list defaults to this rather
// than a hardcoded "yesterday", so it still shows the last real sync
// even if last night's run failed or hasn't happened yet.
export async function getLatestMileageDay() {
  const row = await q1(`SELECT max(day) AS day FROM vehicle_mileage_daily`);
  return { success: true, day: row?.day ? D(row.day) : null };
}

export async function getVehicleMileageDaily({ day } = {}) {
  const targetDay = day || (await getLatestMileageDay()).day;
  if (!targetDay) return { success: true, day: null, data: [] };
  const rows = await q(
    `SELECT plate, distance_m, over_limit, synced_at FROM vehicle_mileage_daily WHERE day=$1 ORDER BY distance_m DESC NULLS LAST`,
    [targetDay]
  );
  return {
    success: true,
    ...
    data: rows.map((r) => ({
      plate: r.plate,
      distanceKm: r.distance_m != null ? Number(r.distance_m) / 1000 : null,
      overLimit: String(r.over_limit).trim().toUpperCase() === "TRUE",
    })),
  };
}

// Small, cheap summary for the Dashboard widget — avoids shipping the
// full per-car list just to show one headline number.
export async function getMileageAlertSummary() {
  const latest = await getLatestMileageDay();
  if (!latest.day) return { success: true, day: null, overLimitCount: 0, carsWithData: 0 };
  const row = await q1(
    `SELECT count(*) FILTER (WHERE over_limit='TRUE') AS over_limit_count, count(*) AS total
       FROM vehicle_mileage_daily WHERE day=$1`,
    [latest.day]
  );
  return {
    success: true, day: latest.day,
    overLimitCount: Number(row?.over_limit_count) || 0,
    carsWithData: Number(row?.total) || 0,
  };
}

// Historical dates a user can actually pick — no point offering a date
// picker with no floor/ceiling when the sync has only ever run since
// whenever this table started filling.
export async function getAvailableMileageDays() {
  const rows = await q(`SELECT DISTINCT day FROM vehicle_mileage_daily ORDER BY day DESC`);
  return { success: true, days: rows.map((r) => D(r.day)) };
}

// Main table for the redesigned Tracking page — one row per confirmed
// car, joining the plate<->tracker pairing with a chosen day's mileage
// (defaults to the latest day when none given) and the last-synced
// location. Deliberately only confirmed cars — unmatched trackers/cars
// have nothing to show here and stay in the review sections.
export async function getTrackerOverviewTable({ day } = {}) {
  const latest = await getLatestMileageDay();
  const targetDay = day || latest.day;
  const [confirmed, mileageRows, locationRows] = await Promise.all([
    q(`SELECT plate, imei, device_name, confirmed_by, confirmed_at FROM vehicle_tracker_map ORDER BY plate`),
    targetDay
      ? q(`SELECT plate, distance_m, over_limit FROM vehicle_mileage_daily WHERE day=$1`, [targetDay])
      : Promise.resolve([]),
    q(`SELECT plate, lat, lng, speed_kmh, acc_on, updated_at FROM vehicle_location_latest`),
  ]);

  const mileageByPlate = new Map(mileageRows.map((r) => [r.plate, r]));
  const locationByPlate = new Map(locationRows.map((r) => [r.plate, r]));

  const data = confirmed.map((c) => {
    const m = mileageByPlate.get(c.plate);
    const loc = locationByPlate.get(c.plate);
    return {
      plate: c.plate,
      imei: c.imei,
      deviceName: S(c.device_name),
      distanceKm: m && m.distance_m != null ? Number(m.distance_m) / 1000 : null,
      overLimit: m ? String(m.over_limit).trim().toUpperCase() === "TRUE" : false,
      // Current position is always "latest known", regardless of which
      // day's mileage is being viewed — only mileage is historized, not
      // location. Showing it alongside a past date's KM would be
      // misleading if presented as "location on that day", so the
      // frontend is expected to only show this column for the latest day.
      lat: loc && loc.lat != null ? Number(loc.lat) : null,
      lng: loc && loc.lng != null ? Number(loc.lng) : null,
      speedKmh: loc && loc.speed_kmh != null ? Number(loc.speed_kmh) : null,
      accOn: loc ? String(loc.acc_on).trim().toUpperCase() === "TRUE" : null,
      locationUpdatedAt: loc ? T(loc.updated_at) : null,
    };
  });

  return { success: true, mileageDay: targetDay, isLatestDay: targetDay === latest.day, data };
}
