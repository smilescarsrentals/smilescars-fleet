...
import { q, q1, S, D, T, todayTZ, requireHRViewAccess, requireHRViewOrCOOAccess, HR_ENABLED_LOCATIONS, requireWorkflowViewAccess } from "./core.js";
...
export async function getStaffList() {
  const rows = await q(`SELECT value, role, phone, COALESCE(active,'TRUE') AS active, COALESCE(receives_all_reservation_reminders,'FALSE') AS receives_all_reservation_reminders, COALESCE(receives_driver_document_reminders,'FALSE') AS receives_driver_document_reminders, COALESCE(can_manage_drivers,'FALSE') AS can_manage_drivers, COALESCE(can_issue_fuel_voucher,'FALSE') AS can_issue_fuel_voucher, COALESCE(location,'') AS location, COALESCE(hr_access,'None') AS hr_access, COALESCE(is_coo,'FALSE') AS is_coo, COALESCE(workflow_access,'None') AS workflow_access FROM config WHERE type='Staff' AND value<>''`);
  return {
    success: true,
    staff: rows.map((r) => ({
