...
export async function getStaffList() {
  const rows = await q(`SELECT value, role, COALESCE(active,'TRUE') AS active, COALESCE(receives_all_reservation_reminders,'FALSE') AS receives_all_reservation_reminders FROM config WHERE type='Staff' AND value<>''`);
  return {
    success: true,
    staff: rows.map((r) => ({
      name: r.value, role: r.role || "Staff", active: String(r.active).trim().toUpperCase() !== "FALSE",
