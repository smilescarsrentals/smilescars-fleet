...
const mapDriver = (r) => ({
  ...
  nationalId: S(r.national_id), address: S(r.address), notes: S(r.notes), photoFileId: S(r.photo_file_id),
  active: String(r.active).trim().toUpperCase() !== "FALSE",
  createdAt: T(r.created_at), updatedAt: T(r.updated_at),
});
...
export async function getAllDriverDocuments() {
  const rows = await q(`SELECT * FROM driver_documents ORDER BY expiry_date ASC NULLS LAST, created_at DESC`);
  return { success: true, data: rows.map(mapDriverDocument) };
}
