...
// Append a row to History (surrogate id auto-fills). Returns the new row's
// id — callers that don't need it (the majority) just ignore it; markReturned
// uses it to link an uploaded fuel photo to this specific return event.
export async function addHistory(e) {
  const row = await insert("history", {
    // Defaults to "now" for the overwhelming majority of entries (logged
    // as they happen), but callers can pass a real event time when
    // something is being logged after the fact — e.g. a 2am Transfer
    // entered into the system at 9am shouldn't be recorded as if it
    // happened at 9am. See checkOut/markReturned's actualTimestamp.
    timestamp: e.timestamp || nowTZ(),
    ...
    is_replacement: e.isReplacement ? "TRUE" : "FALSE",
...
export async function requireAdmin(name) {
  const row = await staffRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role !== "Admin")
...
// Fuel voucher/receipt handling: Admin always allowed; everyone else needs
// the can_issue_fuel_voucher flag explicitly granted in Admin Panel's Staff
// tab — same shape as requireDriverManageAccess above, for the same reason
// (this is a specific-people permission, not a role-wide one).
export async function requireFuelVoucherAccess(name) {
  const row = await q1(
    `SELECT role, COALESCE(active,'TRUE') AS active, COALESCE(can_issue_fuel_voucher,'FALSE') AS can_issue_fuel_voucher
       FROM config WHERE type='Staff' AND btrim(value)=btrim($1) LIMIT 1`,
    [name]
  );
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const canIssue = row ? String(row.can_issue_fuel_voucher).trim().toUpperCase() === "TRUE" : false;
  if (!canIssue) throw new Error("You don't have permission to handle fuel vouchers or receipts.");
}

// HR module — three separate gates, since they're genuinely different
// grants: viewing/managing OTHER people's HR records needs hr_access
// (View sees, Edit manages), while giving the COO-level sign-off on a leave
// request needs is_coo specifically — someone could have one without the
// other. Admin always passes all three. Submitting your OWN leave request
// is intentionally NOT gated by any of these — that's covered separately
// by "is this Dar-tagged active staff", since it's a personal action, not
// an HR-management one.
async function staffHrRow(name) {
  return q1(
    `SELECT role, location, COALESCE(active,'TRUE') AS active, COALESCE(hr_access,'None') AS hr_access, COALESCE(is_coo,'FALSE') AS is_coo
       FROM config WHERE type='Staff' AND btrim(value)=btrim($1) LIMIT 1`,
    [name]
  );
}
export async function requireHRViewAccess(name) {
  const row = await staffHrRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const access = row ? String(row.hr_access).trim() : "None";
  if (access !== "View" && access !== "Edit") throw new Error("You don't have access to the HR module.");
}
export async function requireHREditAccess(name) {
  const row = await staffHrRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const access = row ? String(row.hr_access).trim() : "None";
  if (access !== "Edit") throw new Error("You don't have edit access to the HR module.");
}
export async function requireCOOAccess(name) {
  const row = await staffHrRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const isCoo = row ? String(row.is_coo).trim().toUpperCase() === "TRUE" : false;
  if (!isCoo) throw new Error("Only the COO can give this approval.");
}
// Leave requests need a broader read gate than plain HR access: the COO
// needs to see the queue to do their sign-off even if they hold no general
// HR view/edit grant. Staff profiles stay behind the stricter
// requireHRViewAccess only — COO-only access doesn't imply seeing everyone's
// personal records, just the leave queue.
export async function requireHRViewOrCOOAccess(name) {
  const row = await staffHrRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const access = row ? String(row.hr_access).trim() : "None";
  const isCoo = row ? String(row.is_coo).trim().toUpperCase() === "TRUE" : false;
  if (access !== "View" && access !== "Edit" && !isCoo) throw new Error("You don't have access to the HR module.");
}

// Deleting a leave request outright (any status) is a step above the
// normal HR review flow — restricted to people who can actually act on
// requests (Edit, not View) or the COO, not just anyone with read access.
export async function requireHREditOrCOOAccess(name) {
  const row = await staffHrRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const access = row ? String(row.hr_access).trim() : "None";
  const isCoo = row ? String(row.is_coo).trim().toUpperCase() === "TRUE" : false;
  if (access !== "Edit" && !isCoo) throw new Error("You don't have permission to delete leave requests.");
}

// Mirrors src/components/ActionModal.jsx's HR_ENABLED_LOCATIONS — kept as a
// separate backend copy since this file can't import frontend JSX. Update
// both together if the pilot ever expands to another branch.
export const HR_ENABLED_LOCATIONS = ["Dar es Salaam"];

// Workflows module (invoice approvals) — a single tiered field rather than
// separate booleans, since the roles are naturally ordered: Approve implies
// everything Upload can do, Upload implies everything View can do. Admin
// always passes every tier, same as every other permission in this file.
const WORKFLOW_TIER = { None: 0, View: 1, Upload: 2, Approve: 3 };
async function staffWorkflowRow(name) {
  return q1(
    `SELECT role, COALESCE(active,'TRUE') AS active, COALESCE(workflow_access,'None') AS workflow_access
       FROM config WHERE type='Staff' AND btrim(value)=btrim($1) LIMIT 1`,
    [name]
  );
}
async function requireWorkflowAccess(name, minTier) {
  const row = await staffWorkflowRow(name);
  if (row && !staffActive(row)) throw new Error("This account has been deactivated.");
  const role = row ? String(row.role || "Staff").trim() : "Staff";
  if (role === "Admin") return;
  const tier = WORKFLOW_TIER[row ? row.workflow_access : "None"] ?? 0;
  if (tier < minTier) throw new Error("You don't have access to Workflows.");
}
export async function requireWorkflowViewAccess(name) { return requireWorkflowAccess(name, WORKFLOW_TIER.View); }
export async function requireWorkflowUploadAccess(name) { return requireWorkflowAccess(name, WORKFLOW_TIER.Upload); }
export async function requireWorkflowApproveAccess(name) { return requireWorkflowAccess(name, WORKFLOW_TIER.Approve); }
...
const PUSH_ENABLED_TYPES = new Set([
  ...
  "cover_note_expiry",
  "leave_revoked",
  "workflow_invoice_uploaded",
  "workflow_invoice_reviewed",
]);
