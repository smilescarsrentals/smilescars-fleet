...
import { q, run, todayTZ, createNotification, notifyRole } from "./core.js";
...
// Reservations have no persisted link to the actual checkout event —
// staff mostly skip the "Check Out from Fleet" shortcut on a reservation
// and just check the car out directly from Fleet, which leaves the
// reservation sitting at status='Active' forever even after the car was
// genuinely picked up. Layout.jsx's upcoming-pickup banner already works
// around this live, by cross-referencing Fleet on the fly (matching plate
// + client name) — this does the same match, but persists it, so the
// reservation actually clears from Dashboard/reporting/any other view too,
// not just that one banner. A 1-day grace window on the early side allows
// for a slightly-early pickup; no cap on the late side, since a delayed
// pickup should still reconcile however many days later it happens.
export async function reconcileFulfilledReservations() {
  const n = await run(
    `UPDATE reservations r
        SET status = 'Fulfilled'
      WHERE r.status = 'Active'
        AND r.plate IS NOT NULL AND r.plate <> ''
        AND r.pickup_date <= now()
        AND EXISTS (
          SELECT 1 FROM history h
           WHERE h.plate = r.plate
             AND h.action = 'Checked Out'
             AND btrim(lower(h.client)) = btrim(lower(r.client_name))
             AND h.timestamp >= r.pickup_date - interval '1 day'
        )`
  );
  return { fulfilled: n };
}
...
// Insurance cover note expiry — same shape as checkDriverDocumentExpiry
// above (30 and 7 day reminders, deduped by document+threshold+recipient),
// to staff flagged receives_insurance_reminders. Only the CURRENT cover
// note per plate (furthest expiry) is checked — an older, already-
// superseded note for the same car shouldn't fire its own expiry alert.
export async function checkCoverNoteExpiry() {
  const today = todayTZ();
  const dueRows = await q(
    `SELECT * FROM (
       SELECT DISTINCT ON (plate) id, plate, expiry_date, insurer_name
         FROM car_insurance_cover_notes
        WHERE expiry_date IS NOT NULL
        ORDER BY plate, expiry_date DESC
     ) current_notes
     WHERE expiry_date::date IN (
       (date($1) + interval '30 days')::date,
       (date($1) + interval '7 days')::date
     )`,
    [today]
  );
  const recipients = await q(`SELECT value FROM config WHERE type='Staff' AND COALESCE(active,'TRUE')='TRUE' AND COALESCE(receives_insurance_reminders,'FALSE')='TRUE'`);

  let created = 0;
  for (const note of dueRows) {
    const daysLeft = Math.round((new Date(note.expiry_date).getTime() - new Date(today).getTime()) / 86400000);
    const title = `Insurance cover note for ${note.plate} expires in ${daysLeft} days`;
    const message = `Expires ${note.expiry_date}.${note.insurer_name ? ` (${note.insurer_name})` : ""}`;
    for (const s of recipients) {
      const id = await createNotification({
        recipient: s.value, type: "cover_note_expiry", title, message, linkPath: `/car/${encodeURIComponent(note.plate)}`,
        dedupeKey: `cover_note_expiry:${note.id}:${daysLeft}:${s.value}`,
      });
      if (id) created++;
    }
  }
  return { checked: dueRows.length, created };
}
