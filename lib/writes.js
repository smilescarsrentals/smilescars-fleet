...
export async function addMaintenanceLog(body) {
  ...
  const now = nowTZ();
  // Sequential, month-scoped reference: SC/GAR/2026/08/0001, resets each month.
  // Same pattern as reservations' RES/SC/YYYY/MM/N, but zero-padded to 4 digits.
  ...
  const refNo = prefix + String(maxNum + 1).padStart(4, "0");
