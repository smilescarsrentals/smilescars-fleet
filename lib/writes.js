...
import { extractCoverNoteInfo } from "./coverNoteScan.js";
...
export async function checkOut(body) {
  ...
  if (!body.fuelOut) throw new Error("Fuel Out is required");
  if (!String(body.kmOut || "").trim()) throw new Error("KM Out is required");
  if (!body.fuelPhotoBase64) throw new Error("A photo of the fuel gauge is required");
  ...
  try {
    ...
    // Same pattern as markReturned's fuel-gauge photo — linked to this
    // specific checkout event, not the car's general static photosUrl.
    // Kept inside this same tolerant try (rather than a hard failure like
    // markReturned's) since it depends on historyId existing, and a
    // history-logging hiccup already means this step has nothing to link to.
    if (body.fuelPhotoBase64) {
      const buf = Buffer.from(body.fuelPhotoBase64, "base64");
      const stored = await storeFile({
        kind: "checkout-fuel-photo", filename: body.fuelPhotoFilename || "fuel.jpg",
        mimeType: body.fuelPhotoMimeType || "image/jpeg", buffer: buf, createdBy: body.staffName,
      });
      await insert("history_fuel_photos", {
        id: "HFP-" + crypto.randomBytes(8).toString("hex").toUpperCase(),
        history_id: historyId, file_id: stored.fileId, uploaded_by: body.staffName,
      });
    }
  } catch (err) {
...
export async function logMultiLegTransfer(body) {
  ...
  if (!body.client) throw new Error("Client name is required.");
  if (!body.staffName) throw new Error("Staff name is required.");
  if (!body.fuelOut) throw new Error("Fuel is required.");
  if (!String(body.kmOut || "").trim()) throw new Error("KM Out is required.");
...
export async function markReturned(body) {
  ...
  if (!body.plate) throw new Error("Plate is required");
  if (!body.staffName) throw new Error("Staff name is required");
  // Mirrors ActionModal's client-side required-field check — enforced again
  // here so a direct/bypassed API call can't skip what the return form makes
  // mandatory (KM in, fuel level, fines, remarks, and a fuel-gauge photo).
  if (!body.actualReturn) throw new Error("Returned date is required");
  if (!String(body.kmIn || "").trim()) throw new Error("KM In is required");
  if (!body.fuelIn) throw new Error("Fuel In is required");
  if (body.policeFine === undefined || body.policeFine === null || String(body.policeFine).trim() === "")
    throw new Error("Police Fine is required (enter 0 if none)");
  if (body.parkingFine === undefined || body.parkingFine === null || String(body.parkingFine).trim() === "")
    throw new Error("Parking Fine is required (enter 0 if none)");
  if (!String(body.remarks || "").trim()) throw new Error("Remarks / Notes is required");
  if (body.paymentStatus === "Partial Paid" && !String(body.amountPaid || "").trim())
    throw new Error("Amount paid is required for Partial Paid");
  if (!body.fuelPhotoBase64) throw new Error("A photo of the fuel gauge is required");
  ...
  // Fuel-gauge photo, linked to this specific return event (not the car's
  // general photosUrl, which is a single static link) so every return has
  // its own dated proof of the fuel level reported.
  const buf = Buffer.from(body.fuelPhotoBase64, "base64");
  const stored = await storeFile({
    kind: "return-fuel-photo", filename: body.fuelPhotoFilename || "fuel.jpg",
    mimeType: body.fuelPhotoMimeType || "image/jpeg", buffer: buf, createdBy: body.staffName,
  });
  await insert("history_fuel_photos", {
    id: "HFP-" + crypto.randomBytes(8).toString("hex").toUpperCase(),
...
export async function sendRentedCarToMaintenance(body) {
  ...
  const { refNo, workOrderId } = await createGarageWorkOrderFromFleet({
  ...
  await addHistory({
    ...
    kmOut: body.kmOut, fuelOut: body.fuelOut, workOrderId,
  });
...
export async function setMaintenance(body) {
  ...
  // other half of Phase 2c, intentionally not built yet). Created before
  // the history entry below so that entry can link straight to it (Car
  // Profile's Maintenance Log tab jumps to the matching Garage work order).
  const { refNo, workOrderId } = await createGarageWorkOrderFromFleet({
    plate: body.plate, staffName: body.staffName, remarks: body.remarks,
    serviceLocationType: body.serviceLocationType, internalLocation: body.internalLocation, externalVendorId: body.externalVendorId, externalVendorLocation: body.externalVendorLocation,
  });

  await addHistory({ plate: body.plate, type: body.type || "", action: "Sent to Maintenance", remarks: body.remarks || "", location: clearExtra.location || "", staffName: body.staffName, garage: garageLabel, kmOut: body.kmOut, fuelOut: body.fuelOut, workOrderId });
...
export async function setAvailable(body) {
  ...
  await clearFleetRow(body.plate, "Available", { remarks: body.remarks || "" });
  // Link to whatever work order was open for this plate, if any — Fleet's
  // plain Mark Available doesn't touch the work order itself (that's the
  // Garage module's own Mark Available flow, which closes it directly),
  // but the history entry can still point at it for Car Profile's benefit.
  const openOrder = await q1(`SELECT id FROM maintenance_log WHERE plate=$1 AND status <> 'Completed' ORDER BY date_opened DESC LIMIT 1`, [body.plate]);
  await addHistory({ plate: body.plate, type: body.type || "", action: "Marked Available", remarks: body.remarks || "", location: body.location || "", staffName: body.staffName, kmOut: body.kmOut || "", workOrderId: openOrder?.id });
  return { success: true };
}

// Change which garage a car currently at Maintenance is assigned to,
// without needing to Mark Available and re-send it. Keeps the linked open
// work order (always created alongside Maintenance status — see
// createGarageWorkOrderFromFleet) in sync so the Fleet row and the Garage
// module's own record of the job can't drift apart, the same failure mode
// replaceVehicle used to have before it shared that helper.
export async function changeFleetGarage(body) {
  if (!body.plate) throw new Error("Plate is required");
  if (!body.staffName) throw new Error("Staff name is required");
  ...
  const car = await q1(`SELECT status, garage FROM fleet WHERE plate=$1`, [body.plate]);
  if (!car) throw new Error("Car not found: " + body.plate);
  if (car.status !== "Maintenance") throw new Error("This car isn't currently at a garage.");
  const oldGarageLabel = car.garage || "";

  let newGarageLabel = isExternal ? "" : (body.internalLocation || "SmilesCars Garage");
  if (isExternal) {
    const v = await q1(`SELECT name FROM vendors WHERE id=$1`, [body.externalVendorId]);
    newGarageLabel = v ? v.name : "External garage";
  }

  await run(`UPDATE fleet SET garage=$1 WHERE plate=$2`, [newGarageLabel, body.plate]);

  const workOrder = await q1(`SELECT id FROM maintenance_log WHERE plate=$1 AND status <> 'Completed' ORDER BY date_opened DESC LIMIT 1`, [body.plate]);
  if (workOrder) {
    await run(
      `UPDATE maintenance_log SET service_location_type=$1, internal_location=$2, external_vendor_id=$3, external_vendor_location=$4 WHERE id=$5`,
      [body.serviceLocationType, !isExternal ? (body.internalLocation || "SmilesCars Garage") : "", isExternal ? (body.externalVendorId || null) : null, isExternal ? (body.externalVendorLocation || null) : null, workOrder.id]
    );
  }

  await addHistory({
    plate: body.plate, type: body.type || "", action: "Garage Changed",
    remarks: `${oldGarageLabel || "—"} → ${newGarageLabel}`, garage: newGarageLabel,
    location: !isExternal ? realFleetLocation(newGarageLabel) : "", staffName: body.staffName, workOrderId: workOrder?.id,
  });
  return { success: true };
}
...
export async function replaceVehicle(body) {
  ...
  if (body.originalAction === "available") {
    ...
    // Created before the history entry so it can link straight to it.
    const { workOrderId } = await createGarageWorkOrderFromFleet({
      plate: body.originalPlate, staffName: body.staffName, remarks: origNote,
      serviceLocationType: body.serviceLocationType, internalLocation: body.internalLocation, externalVendorId: body.externalVendorId, externalVendorLocation: body.externalVendorLocation,
    });

    await addHistory({ plate: body.originalPlate, type: body.originalType || "", action: "Sent to Maintenance", remarks: origNote, garage: garageLabel, location: clearExtra.location || "", staffName: body.staffName, kmOut: body.kmOut, fuelOut: body.fuelOut, workOrderId });
  }
  ...
  await addHistory({
    ...
    currency: body.currency || "TZS", paymentStatus: body.paymentStatus || "Paid", amountPaid: body.amountPaid || "",
    driver: body.driver || "", fuelOut: "", isReplacement: true,
  });
  return { success: true };
}

// ── Staff / Config ──────────────────────────────────────────────────────────
// Reachable from both Admin Panel's Staff tab and HR Management's Staffs
// tab now — requireHREditAccess covers both (it passes Admin through
// automatically), rather than leaving this ungated now that it's reachable
// by hr_access=Edit users who aren't necessarily Admin/Manager.
export async function addStaff(body) {
  await requireHREditAccess(body.staffName);
  if (!body.name) throw new Error("Name is required");
  await insert("config", { type: "Staff", value: body.name, password: body.password || "", role: body.role || "Staff", phone: body.phone || "", active: "TRUE", location: body.location || "" });
  return { success: true };
}
...
export async function setReceivesInsuranceReminders(body) {
  await requireManagerOrAdmin(body.staffName);
  if (!body.name) throw new Error("Staff name is required");
  const n = await run(
    `UPDATE config SET receives_insurance_reminders=$1 WHERE type='Staff' AND btrim(value)=btrim($2)`,
    [body.enabled ? "TRUE" : "FALSE", body.name]
  );
  if (!n) throw new Error("Staff not found: " + body.name);
  return { success: true };
...
// Edit an existing leave record's dates — HR/Admin correcting or
// backfilling with the real dates. Recomputes days_requested from the new
// range so the leave balance calculation (which sums days_requested, not
// the dates themselves) stays correct.
export async function editLeaveRequestDates(body) {
  await requireHREditAccess(body.staffName);
  if (!body.requestId) throw new Error("Request ID is required");
  if (!body.startDate || !body.endDate) throw new Error("Start and end dates are required");
  const start = new Date(body.startDate), end = new Date(body.endDate);
  ...
  const daysRequested = Math.round((end - start) / 86400000) + 1;
  const n = await run(
    `UPDATE staff_leave_requests SET start_date=$1, end_date=$2, days_requested=$3 WHERE id=$4`,
    [body.startDate, body.endDate, daysRequested, body.requestId]
  );
  if (!n) throw new Error("Leave request not found.");
  return { success: true };
}
...
// Runs on a single already-sliced 2-page PDF chunk (splitting the source
// PDF into 2-page pieces happens client-side, via pdf-lib, before this is
// ever called — keeps each upload small and keeps this function simple:
// scan one document, store one preview, done). Nothing is filed under any
// car yet — see confirmCoverNote.
export async function scanCoverNote(body) {
  await requireAdmin(body.staffName);
  if (!body.fileBase64) throw new Error("A PDF file is required");
  const buf = Buffer.from(body.fileBase64, "base64");
  const stored = await storeFile({ kind: "cover-note-preview", filename: body.filename || "cover-note.pdf", mimeType: "application/pdf", buffer: buf, createdBy: body.staffName });
  const extracted = await extractCoverNoteInfo({ fileBase64: body.fileBase64, mimeType: "application/pdf" });
  return { success: true, previewFileId: stored.fileId, previewFileUrl: stored.url, ...extracted };
}

// Files a previewed cover note under one or more cars (a single note can
// cover more than one vehicle). Not de-duplicating/reference-counting the
// underlying file across multiple plates -- keeps this simple for now,
// since this is a low-frequency, small-scale operation; a future
// delete-cover-note feature would need to check for other rows sharing
// the same file_id before actually deleting the file.
export async function confirmCoverNote(body) {
  await requireAdmin(body.staffName);
  if (!body.previewFileId) throw new Error("Preview file is required");
  if (!Array.isArray(body.plates) || body.plates.length === 0) throw new Error("At least one car must be selected");
  // Was stored under kind='cover-note-preview' while still under review —
  // now that it's actually being filed, relabel it so a look at the files
  // table itself doesn't misleadingly suggest this is still a temp preview.
  await run(`UPDATE files SET kind='car-insurance-cover-note' WHERE id=$1`, [body.previewFileId]);
  const ids = [];
  for (const plate of body.plates) {
    const id = "CN-" + crypto.randomUUID().split("-")[0].toUpperCase();
    await insert("car_insurance_cover_notes", {
      id, plate, file_id: body.previewFileId, cover_note_number: body.coverNoteNumber || "",
      insurer_name: body.insurerName || "", start_date: body.startDate || null, expiry_date: body.expiryDate || null,
      uploaded_by: body.staffName,
    });
    ids.push(id);
  }
...
export async function discardCoverNotePreview(body) {
  await requireAdmin(body.staffName);
  if (!body.previewFileId) throw new Error("Preview file is required");
  await deleteFile(body.previewFileId).catch(() => {});
  return { success: true };
}
...
export async function markCarAvailable(body) {
  ...
  if (body.outcome === "client") {
    ...
    const workOrder = body.workOrderRef ? await q1(`SELECT id FROM maintenance_log WHERE ref_no=$1`, [body.workOrderRef]) : null;
    await addHistory({
      ...
      staffName: body.staffName, workOrderId: workOrder?.id,
    });
    const resStaff = await findReservationStaffForPlate(body.plate);
    const notifPayload = {
...
// Drivers have no system login (they're out driving, not office staff) --
// leave is requested informally (SMS, WhatsApp, a written letter), so
// whoever manages drivers records it on their behalf: a running "days
// available" number (no fixed annual policy assumed, since driver
// arrangements vary — just a plain number HR sets and maintains directly)
// plus a dated log entry with the proof document attached, so there's a
// record of what was actually shown as evidence.
export async function setDriverLeaveDaysAvailable(body) {
  await requireDriverManageAccess(body.staffName);
  if (!body.driverId) throw new Error("Driver ID is required");
  if (body.leaveDaysAvailable === "" || body.leaveDaysAvailable == null || isNaN(Number(body.leaveDaysAvailable)))
    throw new Error("A valid number of leave days is required");
  const n = await run(`UPDATE drivers SET leave_days_available=$1 WHERE id=$2`, [Number(body.leaveDaysAvailable), body.driverId]);
  if (!n) throw new Error("Driver not found.");
  return { success: true };
}

// Logs a leave instance (with proof) AND decrements the running balance by
// daysUsed in one action, so HR doesn't have to separately recompute and
// re-type the new total by hand each time — the number still ends up
// "updated" exactly as asked, just arithmetically rather than manually,
// which is less error-prone. The proof file is optional (image or PDF —
// could be an SMS/WhatsApp screenshot or a scanned letter) since HR may
// sometimes log something even without a saved copy of the message.
export async function addDriverLeaveRecord(body) {
  await requireDriverManageAccess(body.staffName);
  if (!body.driverId) throw new Error("Driver ID is required");
  if (!body.date) throw new Error("Date is required");
  // The unified Log Leave flow (HR Management) supplies a date range and
  // lets days be computed from it, matching how staff leave works; the
  // Drivers page's own quick-add-with-proof flow still supplies daysUsed
  // directly against a single date. Both are supported here rather than
  // splitting into two functions with duplicate balance-decrement logic.
  let daysUsed;
  if (body.endDate) {
    const start = new Date(body.date), end = new Date(body.endDate);
    if (isNaN(start) || isNaN(end) || end < start) throw new Error("Invalid date range");
    daysUsed = Math.round((end - start) / 86400000) + 1;
  } else {
    daysUsed = Number(body.daysUsed);
    if (isNaN(daysUsed) || daysUsed <= 0) throw new Error("Days used must be a positive number");
  }
  const driver = await q1(`SELECT leave_days_available FROM drivers WHERE id=$1`, [body.driverId]);
  if (!driver) throw new Error("Driver not found.");

  let fileId = "", fileMimeType = "";
  if (body.fileBase64) {
    const buf = Buffer.from(body.fileBase64, "base64");
    const stored = await storeFile({ kind: "driver-leave-proof", filename: body.filename || "proof", mimeType: body.mimeType || "application/octet-stream", buffer: buf, createdBy: body.staffName });
    fileId = stored.fileId;
    fileMimeType = body.mimeType || "";
  }

  const id = "DLV-" + crypto.randomUUID().split("-")[0].toUpperCase();
  await insert("driver_leave_records", {
    id, driver_id: body.driverId, date: body.date, end_date: body.endDate || null, leave_type: body.leaveType || "",
    days_used: daysUsed, file_id: fileId, file_mime_type: fileMimeType,
    notes: body.notes || "", logged_by: body.staffName,
  });

  const currentBalance = driver.leave_days_available != null ? Number(driver.leave_days_available) : 0;
  await run(`UPDATE drivers SET leave_days_available=$1 WHERE id=$2`, [currentBalance - daysUsed, body.driverId]);
  return { success: true, id };
}

// Edit an existing driver leave record's dates. Since driver leave directly
// decrements a running balance (not computed from a policy like staff
// leave), changing the days used here must also adjust
// drivers.leave_days_available by the DIFFERENCE between the old and new
// day count — not just overwrite it — so a correction doesn't silently
// double-count or lose days already reflected in the balance.
export async function editDriverLeaveRecord(body) {
  await requireDriverManageAccess(body.staffName);
  if (!body.recordId) throw new Error("Record ID is required");
  if (!body.date) throw new Error("Date is required");
  const existing = await q1(`SELECT driver_id, days_used FROM driver_leave_records WHERE id=$1`, [body.recordId]);
  ...
  await run(
    `UPDATE driver_leave_records SET date=$1, end_date=$2, days_used=$3 WHERE id=$4`,
    [body.date, body.endDate || null, daysUsed, body.recordId]
  );

  const delta = Number(existing.days_used) - daysUsed; // positive if the new total is smaller
  if (delta !== 0) {
    await run(`UPDATE drivers SET leave_days_available = COALESCE(leave_days_available, 0) + $1 WHERE id=$2`, [delta, existing.driver_id]);
  }
  return { success: true };
}
...
// Open to any staff to record — an accident needs logging quickly by
// whoever's on the scene or takes the call, not gated behind a role.
export async function addAccident(body) {
  if (!body.staffName) throw new Error("Staff name is required");
  if (!body.plate) throw new Error("Plate is required");
  if (!body.accidentDate) throw new Error("Date of accident is required");
  const id = "ACC-" + crypto.randomUUID().split("-")[0].toUpperCase();
  await insert("accidents", {
    id, plate: body.plate, driver_name: body.driverName || "", accident_date: body.accidentDate,
    station_report: body.stationReport || "", injuries_occurred: body.injuriesOccurred ? "TRUE" : "FALSE",
    details: body.details || "", reported_by: body.staffName,
  });
  return { success: true, id };
}

export async function editAccident(body) {
  if (!body.staffName) throw new Error("Staff name is required");
  if (!body.id) throw new Error("Accident ID is required");
  const n = await run(
    `UPDATE accidents SET plate=$1, driver_name=$2, accident_date=$3, station_report=$4, injuries_occurred=$5, details=$6 WHERE id=$7`,
    [body.plate, body.driverName || "", body.accidentDate, body.stationReport || "", body.injuriesOccurred ? "TRUE" : "FALSE", body.details || "", body.id]
  );
  if (!n) throw new Error("Accident record not found.");
  return { success: true };
}

export async function addAccidentFile(body) {
  if (!body.staffName) throw new Error("Staff name is required");
  if (!body.accidentId) throw new Error("Accident ID is required");
  if (!body.fileBase64) throw new Error("A file is required");
  if (!["Photo", "Document"].includes(body.fileType)) throw new Error("Invalid file type");
  const buf = Buffer.from(body.fileBase64, "base64");
  const stored = await storeFile({ kind: "accident-file", filename: body.filename || "file", mimeType: body.mimeType || "application/octet-stream", buffer: buf, createdBy: body.staffName });
  const id = "ACF-" + crypto.randomUUID().split("-")[0].toUpperCase();
  await insert("accident_files", {
    id, accident_id: body.accidentId, file_id: stored.fileId, file_type: body.fileType,
    file_mime_type: body.mimeType || "", filename: body.filename || "", uploaded_by: body.staffName,
  });
  return { success: true, id, fileUrl: stored.url };
}

export async function deleteAccidentFile(body) {
  if (!body.staffName) throw new Error("Staff name is required");
  if (!body.fileRecordId) throw new Error("File record ID is required");
  const row = await q1(`SELECT file_id FROM accident_files WHERE id=$1`, [body.fileRecordId]);
  if (!row) throw new Error("File not found.");
  await run(`DELETE FROM accident_files WHERE id=$1`, [body.fileRecordId]);
  await deleteFile(row.file_id).catch(() => {});
  return { success: true };
}

// Deleting the whole record is more consequential than deleting one
// attached file — kept to Manager/Admin, matching the level several other
// "undo a mistaken entry entirely" actions in this app use.
export async function deleteAccident(body) {
  await requireManagerOrAdmin(body.staffName);
  if (!body.id) throw new Error("Accident ID is required");
  const files = await q(`SELECT file_id FROM accident_files WHERE accident_id=$1`, [body.id]);
  await run(`DELETE FROM accidents WHERE id=$1`, [body.id]); // cascades accident_files
  for (const f of files) await deleteFile(f.file_id).catch(() => {});
  return { success: true };
}
