...
import {
  ...
  updateFleetRow, clearFleetRow, addHistory, staffRow, staffActive, requireManagerOrAdmin, requireMaintenanceEditAccess, realFleetLocation, requireNotGarageManager,
} from "./core.js";
...
export async function sendRentedCarToMaintenance(body) {
  ...
  const fleetUpdate = { status: "Maintenance", garage: garageLabel };
...
export async function setMaintenance(body) {
  ...
  const clearExtra = { remarks: body.remarks || "", garage: garageLabel };
  if (!isExternal) clearExtra.location = realFleetLocation(garageLabel);
  await clearFleetRow(body.plate, "Maintenance", clearExtra);
  await addHistory({ plate: body.plate, type: body.type || "", action: "Sent to Maintenance", remarks: body.remarks || "", location: clearExtra.location || body.location || "", staffName: body.staffName, garage: garageLabel });
...
export async function replaceVehicle(body) {
  ...
  if (body.originalAction === "available") {
    ...
    const clearExtra = { remarks: origNote, garage: garageLabel };
    if (!isExternal) clearExtra.location = realFleetLocation(garageLabel);
    await clearFleetRow(body.originalPlate, "Maintenance", clearExtra);
    await addHistory({ plate: body.originalPlate, type: body.originalType || "", action: "Sent to Maintenance", remarks: origNote, garage: garageLabel, location: clearExtra.location || body.location || "", staffName: body.staffName });
