...
export async function addFuel(body) {
  ...
  // Derive the next ref from the highest existing one rather than the row count,
  // so a deleted row can never cause a duplicate ref.
  const last = await q1(
    `SELECT ref_no FROM fuel WHERE ref_no LIKE 'SC-FUEL-%'
      ORDER BY NULLIF(regexp_replace(ref_no, '\\D', '', 'g'), '')::bigint DESC NULLS LAST LIMIT 1`
  );
  const lastNum = last ? parseInt(String(last.ref_no).replace(/\D/g, ""), 10) : 0;
  const refNo = "SC-FUEL-" + String((isNaN(lastNum) ? 0 : lastNum) + 1).padStart(4, "0");
...
export async function addMaintenanceLog(body) {
  ...
  await insert("maintenance_log", {
    id, plate: body.plate, opened_by: body.openedBy || "", assigned_mechanic: body.assignedMechanic,
...
export async function editMaintenanceLog(body) {
  ...
  if (body.partsCost !== undefined) setObj.parts_cost = body.partsCost;
  if (body.laborCost !== undefined) setObj.labor_cost = body.laborCost;
  ...
  if (body.partsCost !== undefined || body.laborCost !== undefined) {
    const row = await q1(`SELECT parts_cost, labor_cost FROM maintenance_log WHERE id=$1`, [body.id]);
    const parts = body.partsCost !== undefined ? Number(body.partsCost) || 0 : Number(row.parts_cost) || 0;
    const labor = body.laborCost !== undefined ? Number(body.laborCost) || 0 : Number(row.labor_cost) || 0;
    setObj.total_cost = parts + labor;
  }
