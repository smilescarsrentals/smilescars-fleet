...
import { deleteFile, storeFile, readFile } from "./files.js";
import { ONBOARDING_DEFAULT_ITEMS } from "./reads.js";
import { extractWorkflowInvoiceInfo } from "./workflowInvoiceScan.js";
import { autoStraightenAndCropLicense } from "./driverLicenseCleanup.js";
...
// ── Driver license photo cleanup (retroactive review) ───────────────────
// Admin-only: generates a candidate cleaned version of an EXISTING license
// photo without touching the original yet, so Ramzanali can review a
// before/after for each one before anything is replaced (per his call —
// I can't visually verify my own output quality, so nothing gets applied
// without a look first).
export async function previewDriverLicenseCleanup(body) {
  await requireAdmin(body.staffName);
  if (!body.docId) throw new Error("Document ID is required");
  const doc = await q1(`SELECT file_id FROM driver_documents WHERE id=$1`, [body.docId]);
  if (!doc || !doc.file_id) throw new Error("Document or file not found.");
  const original = await readFile(doc.file_id);
  if (!original) throw new Error("Original file not found.");

  const cleaned = await autoStraightenAndCropLicense(original.data, original.mime_type || "image/jpeg");
  const stored = await storeFile({ kind: "driver-license-cleanup-preview", filename: original.filename || "cleaned.jpg", mimeType: "image/jpeg", buffer: cleaned, createdBy: body.staffName });
  return { success: true, previewFileId: stored.fileId, previewFileUrl: stored.url };
}

// Accept: swap the document's file to the previewed version and remove the
// old original — the preview file becomes the new original of record.
export async function applyDriverLicenseCleanup(body) {
  await requireAdmin(body.staffName);
  if (!body.docId || !body.previewFileId) throw new Error("Document ID and preview file are required");
  const doc = await q1(`SELECT file_id FROM driver_documents WHERE id=$1`, [body.docId]);
  if (!doc) throw new Error("Document not found.");
  const oldFileId = doc.file_id;
  await run(`UPDATE driver_documents SET file_id=$1, updated_at=now() WHERE id=$2`, [body.previewFileId, body.docId]);
  if (oldFileId) await deleteFile(oldFileId).catch(() => {});
  return { success: true };
}

// Reject: just delete the unused preview file so it doesn't sit around
// taking up storage — the original document is untouched either way.
export async function discardDriverLicenseCleanupPreview(body) {
  await requireAdmin(body.staffName);
  if (!body.previewFileId) throw new Error("Preview file ID is required");
...
export async function addDriverDocument(body) {
  ...
  let fileId = null;
  if (body.imageBase64) {
    let buf = Buffer.from(body.imageBase64, "base64");
    let mimeType = body.mimeType || "image/jpeg";
    // Auto-straighten + crop driving license photos so they read as a
    // clean scan instead of a tilted photo with a lot of background
    // around them — scoped to this doc type specifically, per
    // Ramzanali's ask, and skipped for non-image files (PDFs).
    if (body.docType === "Driving License" && mimeType.startsWith("image/")) {
      try { buf = await autoStraightenAndCropLicense(buf, mimeType); mimeType = "image/jpeg"; }
      catch { /* keep the original photo if cleanup fails for any reason */ }
    }
    const stored = await storeFile({
      kind: "driver-document", filename: body.filename || "document.jpg",
      mimeType, buffer: buf, createdBy: body.staffName,
    });
    fileId = stored.fileId;
  }
