...
// Only the requester, and only while nothing has reviewed it yet — once HR
// has acted (approved to Pending COO, or rejected), it's no longer a plan
// the requester can quietly withdraw; they'd need to ask HR directly.
export async function cancelLeaveRequest(body) {
  if (!body.staffName) throw new Error("Staff name is required");
  if (!body.requestId) throw new Error("Request ID is required");
  const n = await run(
    `UPDATE staff_leave_requests SET status='Cancelled' WHERE id=$1 AND btrim(staff_name)=btrim($2) AND status='Pending HR'`,
    [body.requestId, body.staffName]
  );
  if (!n) throw new Error("Request not found, not yours, or already reviewed.");
  return { success: true };
}
