...
import {
  ...
  updateFleetRow, clearFleetRow, addHistory, staffRow, staffActive, requireManagerOrAdmin, requireMaintenanceEditAccess, realFleetLocation, requireNotGarageManager, createNotification, notifyRole,
} from "./core.js";
...
export async function setNotificationTriggerEnabled(body) {
  ...
  const n = await run(
    `UPDATE notification_trigger_settings SET enabled=$1, updated_at=now() WHERE type=$2`,
    [body.enabled ? "TRUE" : "FALSE", body.type]
  );
