...
export async function uploadWorkflowInvoice(body) {
  ...
  await insert("workflow_invoices", { id, file_id: fileId, note: body.note || "", status: "Pending", uploaded_by: body.staffName });
...
export async function resubmitWorkflowInvoice(body) {
  ...
  await run(
    `UPDATE workflow_invoices SET file_id=$1, status='Pending', revision=revision+1, reviewed_by='', reviewed_at=NULL, remarks='' WHERE id=$2`,
    [fileId, body.invoiceId]
  );
