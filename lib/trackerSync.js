...
export async function syncVehicleLocations() {
  ...
  try {
    ...
    // TEMP diagnostic — checking whether TrackSolid's raw response includes
    // a reverse-geocoded address field we're not extracting yet (Ramzanali
    // asked to check before deciding between the Google Geocoding API and
    // Nominatim). Remove once answered either way.
    if (locations[0]) console.log("TRACKSOLID_LOCATION_SAMPLE_KEYS:", JSON.stringify(locations[0]));
  } catch (err) {
