...
// and the storage threshold check. Every run logs itself to
// system_health_log (success or failure) so "is this actually running" is
// answerable from data rather than assumed from silence.
//
...
import { checkReservationReminders, checkUnpaidCustomerJobs, checkDisabledTriggers, checkStorageUsage } from "../lib/notificationTriggers.js";
...
export default async function handler(req, res) {
  ...
  try {
    const [reminders, unpaid, disabledTriggers, storage] = await Promise.all([
      checkReservationReminders(),
      checkUnpaidCustomerJobs(),
      checkDisabledTriggers(),
      checkStorageUsage(),
    ]);
    await logHealth("success", `reminders:${reminders.created} unpaid:${unpaid.created} disabledTriggers:${disabledTriggers.created} storageMB:${storage.mb.toFixed(0)}`);
    return res.status(200).json({ success: true, reminders, unpaid, disabledTriggers, storage });
  } catch (err) {
