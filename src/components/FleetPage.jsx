...
export default function FleetPage({ staffName, role }) {
  ...
  // Step 1 -> Step 2 of the checkout wizard: persists the checkout immediately
  // (car is marked Rented right away), then hands the payload to the agreement step.
  const submitCheckout = async (fields) => {
    const car = modal.car;
    const payload = { plate: car.plate, type: car.type, staffName, ...fields };
  ...
  return (
    <div>
      ...
      {modal && modal.action === "checkOut" && (
        <CheckoutFlowModal car={modal.car}
          ...
          onSubmitCheckout={submitCheckout} onClose={()=>!saving&&setModal(null)} />
      )}
      {modal && modal.action !== "checkOut" && (
