  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"1.25rem",flexWrap:"wrap",gap:12 }}>
        <div>
          <div style={{ fontSize:22,fontWeight:700,color:"var(--text)" }}>⛔ Blacklist</div>
          <div style={{ fontSize:13,color:"var(--text-muted)",marginTop:2 }}>{entries.length} flagged {entries.length===1?"person":"people"}</div>
        </div>
        <button type="button" className="btn" style={{ background:"var(--red)",color:"#fff" }} onClick={() => setShowAdd(true)}>
          + Add to Blacklist
        </button>
      </div>
      ...
      <div style={{ marginBottom:"1.25rem" }}>
        <input className="sc-search" style={{ width:"100%",maxWidth:360,padding:"9px 12px",fontSize:13,border:"1.5px solid var(--border)",borderRadius:8,boxSizing:"border-box",background:"var(--surface)",color:"var(--text)" }}
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center",padding:"3rem",color:"var(--text-faint)",fontSize:14 }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16 }} className="sc-blacklist-grid">
          {filtered.map(entry => (
            <div key={entry.id} onClick={() => setLightbox(entry)}
              style={{ background:"var(--surface)",borderRadius:12,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,0.08)",cursor:"pointer",border:"1px solid var(--border)",transition:"transform .15s,box-shadow .15s" }}
              ...
              {/* License image */}
              <div style={{ width:"100%",height:180,background:"var(--border-light)",overflow:"hidden",position:"relative" }}>
                {entry.imageUrl ? (
                  <img src={entry.imageUrl} alt="License" style={{ width:"100%",height:"100%",objectFit:"cover" }} />
                ) : (
                  <div style={{ width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"var(--text-faint)" }}>
                    <div style={{ fontSize:40 }}>🪪</div>
                    <div style={{ fontSize:11,marginTop:6 }}>No image</div>
                  </div>
                )}
                {/* Blacklisted banner */}
                <div style={{ position:"absolute",top:0,left:0,right:0,background:"rgba(220,38,38,0.85)",color:"var(--surface)",fontSize:10,fontWeight:700,textAlign:"center",padding:"4px 0",letterSpacing:"1px" }}>
                  ⛔ BLACKLISTED
                </div>
              </div>
              ...
              <div style={{ padding:"12px" }}>
                <div style={{ fontWeight:700,fontSize:14,color:"var(--text)",marginBottom:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{entry.name||"—"}</div>
                {entry.phone && <div style={{ fontSize:12,color:"var(--text-muted)",marginBottom:2 }}>📞 {entry.phone}</div>}
                {entry.licenseNo && <div style={{ fontSize:12,color:"var(--text)",fontFamily:"monospace" }}>🪪 {entry.licenseNo}</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      {lightbox && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}
          ...
          <div style={{ background:"var(--surface)",borderRadius:16,overflow:"hidden",width:480,maxWidth:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column" }}
            onClick={e => e.stopPropagation()}>

            {/* Lightbox image */}
            <div style={{ background:"var(--text)",minHeight:260,display:"flex",alignItems:"center",justifyContent:"center",position:"relative" }}>
              {lightbox.imageUrl ? (
              ...
              <button onClick={() => setLightbox(null)}
                style={{ position:"absolute",top:12,right:12,background:"rgba(255,255,255,0.2)",border:"none",color:"var(--surface)",borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:16 }}>✕</button>
              <div style={{ position:"absolute",top:0,left:0,right:0,background:"rgba(220,38,38,0.8)",color:"var(--surface)",fontSize:11,fontWeight:700,textAlign:"center",padding:"5px 0",letterSpacing:"1px" }}>
                ⛔ BLACKLISTED
              </div>
            </div>
            ...
            <div style={{ padding:"1.25rem" }}>
              <h3 style={{ fontSize:18,fontWeight:700,color:"var(--text)",margin:"0 0 12px" }}>{lightbox.name||"Unknown"}</h3>
              {[
                ...
                <div key={label} style={{ display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid var(--border-light)",fontSize:13 }}>
                  <span style={{ fontWeight:600,color:"var(--text-muted)",fontSize:12,textTransform:"uppercase",letterSpacing:".3px" }}>{label}</span>
              ...
              {canDelete && (
                <button onClick={() => handleDelete(lightbox)}
                  style={{ width:"100%",marginTop:16,padding:"11px",fontSize:14,fontWeight:600,background:"var(--red)",color:"var(--surface)",border:"none",borderRadius:8,cursor:"pointer" }}>
  ...
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        <div style={{ background:"var(--red)",padding:"1rem 1.25rem",borderRadius:"14px 14px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <p style={{ fontSize:16,fontWeight:700,color:"var(--surface)",margin:0 }}>⛔ Add to Blacklist</p>
        ...
        <div style={{ padding:"1.25rem" }}>
          <div style={{ background:"var(--red-bg)",border:"1px solid var(--red-border)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:13,color:"var(--red)" }}>
          ...
          {err      && <p style={{ color:"var(--red)",fontSize:13,margin:"6px 0" }}>{err}</p>}
          {progress && <p style={{ color:"var(--text-muted)",fontSize:12,margin:"6px 0" }}>{progress}</p>}

          <button style={{ width:"100%",padding:"11px",fontSize:14,fontWeight:600,color:"var(--surface)",background:"var(--red)",border:"none",borderRadius:8,cursor:"pointer",marginTop:4,fontFamily:"inherit",opacity:saving?0.65:1 }}
...
const S = {
  ...
  modal:   { background:"var(--surface)",borderRadius:14,width:440,maxWidth:"100%",maxHeight:"92vh",overflow:"auto",boxShadow:"0 8px 40px rgba(0,0,0,0.18)" },
