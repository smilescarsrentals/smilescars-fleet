...
export default function ReservationsPage({ staffName, role }) {
  ...
  return (
    <div>
                ...
                <div style={S.slots}>
                  {slots.map(r => (
                    <div key={r.id} style={{ ...S.card,borderLeft:`3px solid ${colorFor(r.carType||r.client)}` }}
                      ...
                      {r.carType && <span style={{ fontSize:11,color:"#888",marginLeft:5 }}>· {r.carType}</span>}
                      {r.numCars && r.numCars > 1 && <span style={{ fontSize:11,color:"#888",marginLeft:3 }}>×{r.numCars}</span>}
                    </div>
                  ))}
      ...
      {showAdd    && <AddModal    day={showAdd}    month={month} year={year} staffName={staffName} onClose={()=>setShowAdd(null)}    onSaved={()=>{setShowAdd(null);load();}} />}
      ...
      {showEdit   && <EditModal   res={showEdit}   onClose={()=>setShowEdit(null)} onSaved={()=>{setShowEdit(null);load();}} />}
    </div>
  );
}
...
function AddModal({ day, month, year, staffName, onClose, onSaved }) {
  ...
  const displayDate = `${pad(day)} ${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][month-1]} ${year}`;
  const [form,   setForm]   = useState({ client:"", phone:"", carType:"", numCars:"1", pickUpFrom:"", remarks:"" });
  ...
  const handleSave = async () => {
    ...
    try {
      await api.addReservation({ date:dateStr, staffName, ...form });
      onSaved();
        ...
        <div style={S.mBody}>
          ...
          <div style={S.two}>
            <div style={S.field}><label style={S.label}>Car Type</label>
              <input style={S.input} value={form.carType} onChange={e=>set("carType",e.target.value)} placeholder="e.g. Alphard, Harrier" /></div>
            <div style={S.field}><label style={S.label}>No. of Cars</label>
              <input style={S.input} type="number" min="1" value={form.numCars} onChange={e=>set("numCars",e.target.value)} placeholder="1" /></div>
          </div>
          <div style={S.field}><label style={S.label}>Pick Up From</label>
            <input style={S.input} value={form.pickUpFrom} onChange={e=>set("pickUpFrom",e.target.value)} placeholder="e.g. Airport, JNIA, Hotel name" /></div>
          <div style={S.field}><label style={S.label}>Remarks</label>
            <textarea style={S.textarea} rows={2} value={form.remarks} onChange={e=>set("remarks",e.target.value)} placeholder="e.g. Needs child seat, early morning pickup" /></div>
...
function DetailModal({ res, canEdit, onClose, onEdit, onDeleted }) {
  ...
  const displayDate = `${pad(d)} ${["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][m-1]} ${y}`;
      ...
      <div style={S.modal} onClick={e=>e.stopPropagation()}>
        <div style={{ ...S.mHead,background:color }}>
          <div>
            ...
            <p style={S.mSub}>{displayDate}{res.carType?` · ${res.carType}`:""}{res.numCars>1?` × ${res.numCars}`:""}</p>
          </div>
          <button style={S.closeBtn} onClick={onClose}>✕</button>
        </div>
        <div style={S.mBody}>
          {[
            ["Date",         displayDate],
            ["Client Name",  res.client],
            ["Contact No.",  res.phone      ||"—"],
            ["Car Type",     res.carType    ||"—"],
            ["No. of Cars",  res.numCars    ||"1"],
            ...
            <div key={label} style={{ display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #f3f4f6",fontSize:13 }}>
              ...
              <span style={{ color:"#111",textAlign:"right",maxWidth:220 }}>{val}</span>
            </div>
          ))}
          {canEdit && (
...
function EditModal({ res, onClose, onSaved }) {
  const [form,   setForm]   = useState({ date:res.date||"", client:res.client||"", phone:res.phone||"", carType:res.carType||"", numCars:res.numCars||"1", pickUpFrom:res.pickUpFrom||"", remarks:res.remarks||"" });
  const [saving, setSaving] = useState(false);
  const [err,    setErr]    = useState("");
  const set = (k,v) => setForm(f=>({...f,[k]:v}));
  ...
  const handleSave = async () => {
    ...
    setSaving(true); setErr("");
    try {
      await api.editReservation({ id:res.id, ...form });
      onSaved();
    } catch(e) { setErr(e.message); }
    finally { setSaving(false); }
  };
      ...
      <div style={S.modal} onClick={e=>e.stopPropagation()}>
        ...
        <div style={S.mBody}>
          <div style={S.field}><label style={S.label}>Date *</label>
            <input style={S.input} type="date" value={form.date} onChange={e=>set("date",e.target.value)} /></div>
          <div style={S.field}><label style={S.label}>Client Name *</label>
          ...
          <div style={S.two}>
            <div style={S.field}><label style={S.label}>Car Type</label>
              <input style={S.input} value={form.carType} onChange={e=>set("carType",e.target.value)} placeholder="e.g. Alphard" /></div>
            <div style={S.field}><label style={S.label}>No. of Cars</label>
              <input style={S.input} type="number" min="1" value={form.numCars} onChange={e=>set("numCars",e.target.value)} /></div>
          </div>
          <div style={S.field}><label style={S.label}>Pick Up From</label>
...
const S = {
  ...
  card:    { background:"#f0f9ff",borderRadius:6,padding:"4px 10px",cursor:"pointer",display:"flex",alignItems:"center",fontSize:12,gap:4,boxShadow:"0 1px 3px rgba(0,0,0,0.06)" },
