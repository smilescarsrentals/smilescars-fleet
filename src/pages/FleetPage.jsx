...
const STATUS_STYLES = {
  ...
  Garage:      { bg: "#ede9fe", color: "#6d28d9" },
};

export default function FleetPage({ staffName }) {
  const [fleet,     setFleet]     = useState([]);
  const [config,    setConfig]    = useState({ staff: [], locations: [] });
  ...
  const [modal,     setModal]     = useState(null);   // { car, action }
  ...
  const handleConfirm = async ({ client, clientPhone, returnDate, location, remarks, kmOut, kmIn, amount, newLocation }) => {
    if (!modal) return;
    setSaving(true);
    try {
      const { car, action } = modal;
      const payload = { plate: car.plate, type: car.type, staffName, client, clientPhone, returnDate, location, remarks, kmOut, kmIn, amount };
      ...
      if (action === "setGarage")      await api.setGarage(payload);
      if (action === "setAvailable")   await api.setAvailable(payload);

      if (newLocation) await api.addLocation(newLocation);
  ...
  const stats = useMemo(() => ({
    ...
    garage:      fleet.filter(c => c.status === "Garage").length,
  }), [fleet]);
  ...
  const resetFilters = () => { setSearch(""); setFStatus(""); setFLocation(""); setFType(""); setPage(1); };
    ...
    <div>
      ...
      <div style={styles.statsRow}>
        {[
          ...
          { label: "Garage",      value: stats.garage,      color: "#6d28d9", bg: "#ede9fe" },
        ].map(s => (
      ...
      <div style={styles.filterRow}>
        ...
        <select style={styles.sel} value={fStatus} onChange={e => { setFStatus(e.target.value); setPage(1); }}>
          ...
          {["Available","Rented","Maintenance","Garage"].map(s => <option key={s}>{s}</option>)}
        </select>
        ...
        {(search || fStatus || fLocation || fType) &&
          <button style={styles.clearBtn} onClick={resetFilters}>Clear</button>}
      ...
      <div style={styles.tableWrap}>
        <table style={styles.table}>
          <thead>
            <tr>
              {["Plate","Type","Location","Status","Client","Return Date","Remarks","Action"].map(h => (
          ...
          <tbody>
            {paginated.length === 0 && (
              <tr><td colSpan={8} style={styles.empty}>No vehicles match your filters.</td></tr>
            )}
            {paginated.map(car => {
              const ss = STATUS_STYLES[car.status] || STATUS_STYLES.Available;
              return (
                <tr key={car.plate} style={styles.tr}>
                  <td style={{ ...styles.td, fontWeight: 600, fontSize: 13 }}>{car.plate}</td>
                  <td style={styles.td}>{car.type}</td>
                  <td style={styles.td}>
                    {car.location
                      ? <span style={styles.locChip}>{car.location}</span>
                      : <span style={styles.locEmpty}>—</span>}
                  </td>
                  <td style={styles.td}>
                  ...
                  <td style={styles.td}>
                    {car.currentClient
                      ? <div>
                          <div style={{ fontWeight: 500, fontSize: 13 }}>{car.currentClient}</div>
                  ...
                  <td style={{ ...styles.td, fontSize: 12, color: "#777", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
                    title={car.remarks}>{car.remarks || "—"}</td>
                  <td style={styles.td}>
                    <ActionButtons car={car} onAction={handleAction} />
                  </td>
                </tr>
              );
            })}
...
function ActionButtons({ car, onAction }) {
  ...
  if (car.status === "Available") return (
    <div>
      ...
      {btn("Garage",      "setGarage",      "#6d28d9", "#f5f3ff")}
    </div>
  );
  ...
  if (car.status === "Maintenance" || car.status === "Garage") return btn("Mark Available", "setAvailable", "#15803d", "#dcfce7");
  return null;
}
...
const styles = {
  ...
  toast:      { position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)",
                ...
                fontSize: 14, zIndex: 200, boxShadow: "0 4px 16px rgba(0,0,0,0.2)" },
  statsRow:   { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: "1.25rem" },
  statCard:   { borderRadius: 10, padding: "14px 16px", textAlign: "center", cursor: "pointer", transition: "opacity .15s" },
  ...
  th:         { padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#888",
                borderBottom: "1px solid #e5e7eb", background: "#fafafa", textTransform: "uppercase", letterSpacing: ".4px", whiteSpace: "nowrap" },
  ...
  locEmpty:   { color: "#ccc", fontSize: 13 },
