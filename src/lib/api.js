// The backend lives at /api in this same deployment (Supabase-backed — see
// /api/index.js).
//
// VITE_SCRIPT_URL is deliberately IGNORED. Vite inlines env variables at build
// time, so a leftover VITE_SCRIPT_URL in the Vercel project silently sends the
// whole app back to Google Apps Script — which is what it did after the
// migration, until this constant took over. To point the app somewhere else
// (another deployment, or back to Apps Script), edit this line; don't
// reintroduce the env variable.
const SCRIPT_URL = "/api";

if (import.meta.env.VITE_SCRIPT_URL) {
  console.warn(
    `[SmilesCars] Ignoring VITE_SCRIPT_URL (${import.meta.env.VITE_SCRIPT_URL}) — ` +
    `the app reads Supabase through ${SCRIPT_URL}. Delete that variable from the ` +
    `Vercel project when you get the chance; it has no effect either way.`
  );
}

export async function get(action, params = {}) {
  const url = new URL(SCRIPT_URL, window.location.origin); // second arg resolves the relative "/api"
  url.searchParams.set("action", action);
  Object.entries(params).forEach(([k, v]) => url.searchParams.set(k, v));
  const res  = await fetch(url.toString(), { method: "GET", redirect: "follow" });
  const text = await res.text();
  const data = parseJsonOrThrow(text, res.status, action);
  if (data.error) throw new Error(data.error);
  return data;
}

export async function post(body) {
  const res  = await fetch(SCRIPT_URL, {
    method: "POST", redirect: "follow",
    headers: { "Content-Type": "text/plain" },
    body: JSON.stringify(body),
  });
  const text = await res.text();
  const data = parseJsonOrThrow(text, res.status, body.action);
  if (data.error) throw new Error(data.error);
  return data;
}

// A non-JSON response (Vercel's own timeout page, a proxy error, etc.)
// used to surface as a raw, cryptic browser parser error ('Unexpected
// identifier "An"...') with no indication of what actually happened or
// which request it was. This gives a plain-English message instead.
function parseJsonOrThrow(text, status, action) {
  try {
    return JSON.parse(text);
  } catch {
    throw new Error(
      `The server didn't respond properly to "${action}" (HTTP ${status}). ` +
      `This can happen if the request took too long — try again, or with a smaller batch of work.`
    );
  }
}

export const api = {
  getFleet:              ()      => get("getFleet"),
  getAccidents:          ()      => get("getAccidents"),
  getAccidentById:       (id)    => get("getAccidentById", { id }),
  addAccident:           (body)  => post({ action: "addAccident", ...body }),
  editAccident:          (body)  => post({ action: "editAccident", ...body }),
  addAccidentFile:       (body)  => post({ action: "addAccidentFile", ...body }),
  deleteAccidentFile:    (body)  => post({ action: "deleteAccidentFile", ...body }),
  deleteAccident:        (body)  => post({ action: "deleteAccident", ...body }),
  getMonthlyRevenue:     (params) => get("getMonthlyRevenue", params),
  getHistory:            ()      => get("getHistory"),
  getConfig:             ()      => get("getConfig"),
  getSold:               ()      => get("getSold"),
  getSubHire:            ()      => get("getSubHire"),
  getClients:            ()      => get("getClients"),
  getFuel:               ()      => get("getFuel"),
  getFuelByPlate:        (plate) => get("getFuelByPlate", { plate }),
  getCarByPlate:         (plate) => get("getCarByPlate", { plate }),
  getCarHistory:         (plate)       => get("getCarHistory",     { plate }),
  getHistoryByStaff:     (staffName)   => get("getHistoryByStaff", { staffName }),
  getReservations:       (month, year) => get("getReservations", { month, year }),
  getAllReservations:     ()            => get("getAllReservations"),
  getBlacklist:          ()            => get("getBlacklist"),
  uploadBlacklistImage:  (body)        => post({ action: "uploadBlacklistImage", ...body }),
  addToBlacklist:        (body)        => post({ action: "addToBlacklist",       ...body }),
  deleteFromBlacklist:   (body)        => post({ action: "deleteFromBlacklist",  ...body }),
  addReservation:        (body)  => post({ action: "addReservation",    ...body }),
  editReservation:       (body)  => post({ action: "editReservation",   ...body }),
  deleteReservation:     (body)  => post({ action: "deleteReservation", ...body }),
  cancelReservation:     (body)  => post({ action: "cancelReservation", ...body }),
  verifyStaff:           (body)  => post({ action: "verifyStaff",           ...body }),
  checkOut:              (body)  => post({ action: "checkOut",               ...body }),
  logMultiLegTransfer:   (body)  => post({ action: "logMultiLegTransfer",    ...body }),
  addCar:                (body)  => post({ action: "addCar",                 ...body }),
  renameFleetPlate:      (body)  => post({ action: "renameFleetPlate",       ...body }),
  addStaff:              (body)  => post({ action: "addStaff",               ...body }),
  getStaffList:           ()     => get("getStaffList"),
  getHRStaffList:         (staffName) => get("getHRStaffList", { staffName }),
  getHRStaffProfile:      (staffName, targetStaffName) => get("getHRStaffProfile", { staffName, targetStaffName }),
  getMyProfile:           (staffName) => get("getMyProfile", { staffName }),
  saveHRStaffProfile:     (body) => post({ action: "saveHRStaffProfile", ...body }),
  getMyLeaveRequests:     (staffName) => get("getMyLeaveRequests", { staffName }),
  getHRLeaveRequests:     (staffName) => get("getHRLeaveRequests", { staffName }),
  getMyDisciplinaryRecords: (staffName) => get("getMyDisciplinaryRecords", { staffName }),
  getHRDisciplinaryRecords: (staffName, targetStaffName) => get("getHRDisciplinaryRecords", { staffName, targetStaffName }),
  addDisciplinaryRecord:  (body) => post({ action: "addDisciplinaryRecord", ...body }),
  getOnboardingChecklist: (staffName, targetStaffName) => get("getOnboardingChecklist", { staffName, targetStaffName }),
  getWorkflowInvoices:    (staffName) => get("getWorkflowInvoices", { staffName }),
  getWorkflowInvoiceEvents: (staffName, invoiceId) => get("getWorkflowInvoiceEvents", { staffName, invoiceId }),
  getCoverNotesForCar:    (plate) => get("getCoverNotesForCar", { plate }),
  getFleetCoverNoteStatus: (staffName) => get("getFleetCoverNoteStatus", { staffName }),
  seedOnboardingChecklist: (body) => post({ action: "seedOnboardingChecklist", ...body }),
  addOnboardingItem:      (body) => post({ action: "addOnboardingItem", ...body }),
  toggleOnboardingItem:   (body) => post({ action: "toggleOnboardingItem", ...body }),
  deleteOnboardingItem:   (body) => post({ action: "deleteOnboardingItem", ...body }),
  uploadWorkflowInvoice:  (body) => post({ action: "uploadWorkflowInvoice", ...body }),
  resubmitWorkflowInvoice: (body) => post({ action: "resubmitWorkflowInvoice", ...body }),
  reviewWorkflowInvoice:  (body) => post({ action: "reviewWorkflowInvoice", ...body }),
  scanCoverNote:          (body) => post({ action: "scanCoverNote", ...body }),
  confirmCoverNote:       (body) => post({ action: "confirmCoverNote", ...body }),
  discardCoverNotePreview: (body) => post({ action: "discardCoverNotePreview", ...body }),
  submitLeaveRequest:     (body) => post({ action: "submitLeaveRequest", ...body }),
  cancelLeaveRequest:     (body) => post({ action: "cancelLeaveRequest", ...body }),
  reviewLeaveRequestHR:   (body) => post({ action: "reviewLeaveRequestHR", ...body }),
  reviewLeaveRequestCOO:  (body) => post({ action: "reviewLeaveRequestCOO", ...body }),
  deleteLeaveRequest:     (body) => post({ action: "deleteLeaveRequest", ...body }),
  addLeaveOnBehalf:       (body) => post({ action: "addLeaveOnBehalf", ...body }),
  editLeaveRequestDates:  (body) => post({ action: "editLeaveRequestDates", ...body }),
  getDrivers:             ()     => get("getDrivers"),
  setStaffActive:        (body)  => post({ action: "setStaffActive",         ...body }),
  addDriverWithPhone:     (body) => post({ action: "addDriverWithPhone",     ...body }),
  scanInvoice:            (body) => post({ action: "scanInvoice",            ...body }),
  scanFuelReceipt:        (body) => post({ action: "scanFuelReceipt",        ...body }),
  confirmFuelReceipts:    (body) => post({ action: "confirmFuelReceipts",    ...body }),
  confirmInvoiceScan:     (body) => post({ action: "confirmInvoiceScan",     ...body }),
  getPartCostHistory:     (partId) => get("getPartCostHistory", { partId }),
  getDriversV2:           ()     => get("getDriversV2"),
  getDriverById:          (id)   => get("getDriverById", { id }),
  getCurrentGarageAssignment: (plate) => get("getCurrentGarageAssignment", { plate }),
  getDriverLeaveRecords:  (driverId) => get("getDriverLeaveRecords", { driverId }),
  getDriverAssignmentLog: (driverName) => get("getDriverAssignmentLog", { driverName }),
  getDriverCurrentAssignments: () => get("getDriverCurrentAssignments"),
  getDriversByClientHistory: (params) => get("getDriversByClientHistory", params),
  getDriverDocuments:     (driverId) => get("getDriverDocuments", { driverId }),
  getAllDriverDocuments:  ()     => get("getAllDriverDocuments"),
  setReceivesDriverDocumentReminders: (body) => post({ action: "setReceivesDriverDocumentReminders", ...body }),
  setReceivesInsuranceReminders: (body) => post({ action: "setReceivesInsuranceReminders", ...body }),
  setCanManageDrivers:    (body) => post({ action: "setCanManageDrivers",    ...body }),
  setCanIssueFuelVoucher: (body) => post({ action: "setCanIssueFuelVoucher", ...body }),
  setStaffLocation:       (body) => post({ action: "setStaffLocation",       ...body }),
  setHRAccess:            (body) => post({ action: "setHRAccess",            ...body }),
  setIsCoo:               (body) => post({ action: "setIsCoo",               ...body }),
  addDriverV2:            (body) => post({ action: "addDriverV2",            ...body }),
  setDriverLeaveDaysAvailable: (body) => post({ action: "setDriverLeaveDaysAvailable", ...body }),
  addDriverLeaveRecord:   (body) => post({ action: "addDriverLeaveRecord", ...body }),
  editDriverLeaveRecord:  (body) => post({ action: "editDriverLeaveRecord", ...body }),
  bulkAddDrivers:         (body) => post({ action: "bulkAddDrivers",         ...body }),
  bulkAddDriverDocuments: (body) => post({ action: "bulkAddDriverDocuments", ...body }),
  editDriver:              (body) => post({ action: "editDriver",             ...body }),
  setDriverPhoto:          (body) => post({ action: "setDriverPhoto",         ...body }),
  addDriverDocument:       (body) => post({ action: "addDriverDocument",      ...body }),
  editDriverDocument:      (body) => post({ action: "editDriverDocument",     ...body }),
  deleteDriverDocument:    (body) => post({ action: "deleteDriverDocument",   ...body }),
  getTrackerMatchSuggestions: () => get("getTrackerMatchSuggestions"),
  getTrackerMap:           ()     => get("getTrackerMap"),
  getVehicleMileageDaily:  (params) => get("getVehicleMileageDaily", params),
  getMileageAlertSummary:  ()     => get("getMileageAlertSummary"),
  getTrackerOverviewTable: (day)  => get("getTrackerOverviewTable", day ? { day } : {}),
  getAvailableMileageDays: ()     => get("getAvailableMileageDays"),
  confirmTrackerMatches:   (body) => post({ action: "confirmTrackerMatches", ...body }),
  removeTrackerMatch:      (body) => post({ action: "removeTrackerMatch",    ...body }),
  ignoreTrackerDevice:     (body) => post({ action: "ignoreTrackerDevice",   ...body }),
  unignoreTrackerDevice:   (body) => post({ action: "unignoreTrackerDevice", ...body }),
  runTrackerSyncNow:       (body) => post({ action: "runTrackerSyncNow",     ...body }),
  getPurchaseInvoices:    ()     => get("getPurchaseInvoices"),
  getPurchaseInvoiceItems: (invoiceId) => get("getPurchaseInvoiceItems", { invoiceId }),
  getSystemHealth:        ()     => get("getSystemHealth"),
  setStaffPhone:          (body) => post({ action: "setStaffPhone",          ...body }),
  getSettings:            ()     => get("getSettings"),
  updateSetting:         (body)  => post({ action: "updateSetting",          ...body }),
  updateConfigItem:      (body)  => post({ action: "updateConfigItem",       ...body }),
  deleteConfigItem:      (body)  => post({ action: "deleteConfigItem",       ...body }),
  getDropboxSyncStatus:   ()     => get("getDropboxSyncStatus"),
  enableDropboxSync:      ()     => post({ action: "enableDropboxSync" }),
  disableDropboxSync:     ()     => post({ action: "disableDropboxSync" }),
  createBackupSnapshot:   ()     => post({ action: "createBackupSnapshot" }),
  deleteStaffSignature:  (body)  => post({ action: "deleteStaffSignature",   ...body }),
  markReturned:          (body)  => post({ action: "markReturned",           ...body }),
  extendBooking:         (body)  => post({ action: "extendBooking",          ...body }),
  setMaintenance:        (body)  => post({ action: "setMaintenance",         ...body }),
  sendRentedCarToMaintenance: (body) => post({ action: "sendRentedCarToMaintenance", ...body }),
  setAvailable:          (body)  => post({ action: "setAvailable",           ...body }),
  changeFleetGarage:     (body)  => post({ action: "changeFleetGarage",      ...body }),
  setStaffUse:           (body)  => post({ action: "setStaffUse",            ...body }),
  replaceVehicle:        (body)  => post({ action: "replaceVehicle",          ...body }),
  updateLocation:        (body)  => post({ action: "updateLocation",         ...body }),
  updatePayment:         (body)  => post({ action: "updatePayment",          ...body }),
  markSold:              (body)  => post({ action: "markSold",               ...body }),
  addLocation:           (name)  => post({ action: "addLocation",            name }),
  addGarage:             (name)  => post({ action: "addGarage",              name }),
  addDriver:             (name)  => post({ action: "addDriver",              name }),
  addCarNote:            (body)  => post({ action: "addCarNote",             ...body }),
  addFuel:               (body)  => post({ action: "addFuel",                ...body }),
  editFuel:              (body)  => post({ action: "editFuel",               ...body }),
  addSubHire:            (body)  => post({ action: "addSubHire",             ...body }),
  returnSubHire:         (body)  => post({ action: "returnSubHire",          ...body }),
  updateSubHirePayment:  (body)  => post({ action: "updateSubHirePayment",   ...body }),
  getLeads:               ()     => get("getLeads"),
  getVendors:             ()     => get("getVendors"),
  getVendorCategories:    ()     => get("getVendorCategories"),
  addVendorCategory:      (body) => post({ action: "addVendorCategory",      ...body }),
  setVendorCategories:    (body) => post({ action: "setVendorCategories",    ...body }),
  addVendor:              (body) => post({ action: "addVendor",              ...body }),
  editVendor:             (body) => post({ action: "editVendor",             ...body }),
  deleteVendor:           (body) => post({ action: "deleteVendor",           ...body }),
  addVendorLocation:      (body) => post({ action: "addVendorLocation",      ...body }),
  deleteVendorLocation:   (body) => post({ action: "deleteVendorLocation",   ...body }),
  getNotifications:       (staffName) => get("getNotifications", { staffName }),
  getUnreadNotificationCount: (staffName) => get("getUnreadNotificationCount", { staffName }),
  markNotificationRead:   (body) => post({ action: "markNotificationRead",   ...body }),
  markAllNotificationsRead: (body) => post({ action: "markAllNotificationsRead", ...body }),
  getPushSubscriptions:  (staffName) => get("getPushSubscriptions", { staffName }),
  savePushSubscription:  (body) => post({ action: "savePushSubscription", ...body }),
  deletePushSubscription: (body) => post({ action: "deletePushSubscription", ...body }),
  getNotificationTriggerSettings: () => get("getNotificationTriggerSettings"),
  setNotificationTriggerEnabled: (body) => post({ action: "setNotificationTriggerEnabled", ...body }),
  setReceivesAllReservationReminders: (body) => post({ action: "setReceivesAllReservationReminders", ...body }),
  getParts:               ()     => get("getParts"),
  addPart:                (body) => post({ action: "addPart",                ...body }),
  editPart:               (body) => post({ action: "editPart",               ...body }),
  deletePart:             (body) => post({ action: "deletePart",             ...body }),
  getServiceTemplates:    ()     => get("getServiceTemplates"),
  addServiceTemplate:     (body) => post({ action: "addServiceTemplate",     ...body }),
  editServiceTemplate:    (body) => post({ action: "editServiceTemplate",    ...body }),
  deleteServiceTemplate:  (body) => post({ action: "deleteServiceTemplate",  ...body }),
  addTemplatePart:        (body) => post({ action: "addTemplatePart",       ...body }),
  deleteTemplatePart:     (body) => post({ action: "deleteTemplatePart",    ...body }),
  getCarServiceAssignments: ()   => get("getCarServiceAssignments"),
  assignServiceTemplate:  (body) => post({ action: "assignServiceTemplate", ...body }),
  editCarServiceAssignment: (body) => post({ action: "editCarServiceAssignment", ...body }),
  removeServiceAssignment: (body) => post({ action: "removeServiceAssignment", ...body }),
  createWorkOrderFromTemplate: (body) => post({ action: "createWorkOrderFromTemplate", ...body }),
  getChecklistTemplates:  ()     => get("getChecklistTemplates"),
  addChecklistTemplate:   (body) => post({ action: "addChecklistTemplate",   ...body }),
  editChecklistTemplate:  (body) => post({ action: "editChecklistTemplate",  ...body }),
  deleteChecklistTemplate: (body) => post({ action: "deleteChecklistTemplate", ...body }),
  addChecklistTemplateItem: (body) => post({ action: "addChecklistTemplateItem", ...body }),
  deleteChecklistTemplateItem: (body) => post({ action: "deleteChecklistTemplateItem", ...body }),
  getChecklistInstances:  (params) => get("getChecklistInstances", params),
  submitChecklist:        (body) => post({ action: "submitChecklist",        ...body }),
  addLead:                (body) => post({ action: "addLead",                ...body }),
  editLead:               (body) => post({ action: "editLead",               ...body }),
  deleteLead:             (body) => post({ action: "deleteLead",             ...body }),
  getMaintenanceLog:      ()     => get("getMaintenanceLog"),
  addMaintenanceLog:      (body) => post({ action: "addMaintenanceLog",      ...body }),
  editMaintenanceLog:     (body) => post({ action: "editMaintenanceLog",     ...body }),
  getMaintenanceItems:    (workOrderId) => get("getMaintenanceItems", { workOrderId }),
  getPreviousSupplierPrice: (supplierVendorId, itemName) => get("getPreviousSupplierPrice", { supplierVendorId, itemName }),
  getAllMaintenanceItems: ()     => get("getAllMaintenanceItems"),
  addMaintenanceItem:     (body) => post({ action: "addMaintenanceItem",     ...body }),
  editMaintenanceItem:    (body) => post({ action: "editMaintenanceItem",    ...body }),
  deleteMaintenanceItem:  (body) => post({ action: "deleteMaintenanceItem",  ...body }),
  getMaintenanceUpdates:  (workOrderId) => get("getMaintenanceUpdates", { workOrderId }),
  addMaintenanceUpdate:   (body) => post({ action: "addMaintenanceUpdate",   ...body }),
  getCustomerJobs:        ()     => get("getCustomerJobs"),
  addCustomerJob:         (body) => post({ action: "addCustomerJob",         ...body }),
  editCustomerJob:        (body) => post({ action: "editCustomerJob",        ...body }),
  getCustomerJobItems:    (jobId) => get("getCustomerJobItems", { jobId }),
  addCustomerJobItem:     (body) => post({ action: "addCustomerJobItem",     ...body }),
  deleteCustomerJobItem:  (body) => post({ action: "deleteCustomerJobItem",  ...body }),
  getCustomerJobUpdates:  (jobId) => get("getCustomerJobUpdates", { jobId }),
  addCustomerJobUpdate:   (body) => post({ action: "addCustomerJobUpdate",   ...body }),
  markCarAvailable:       (body) => post({ action: "markCarAvailable",       ...body }),
  setServiceSchedule:     (body) => post({ action: "setServiceSchedule",     ...body }),
};
